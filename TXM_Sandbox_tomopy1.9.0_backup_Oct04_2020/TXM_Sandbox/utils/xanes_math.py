...
def tv_l1_pixel(fixed_img, img, shift):
...
def multi_resolutin_tv_reg(fixed_img, img, levs=4, wz=10, sp_wz=20, sp_step=0.2):
    ...
    for ii in range(levs-1): 
        ...
        with mp.get_context().Pool(int(n_cpu-1)) as pool:
            rlt = pool.map(partial(tv_l1_pixel, fixed_img, img), 
                           [[int(step*(jj//w-int(w/2))+shift[:ii, 0].sum()), 
                             int(step*(jj%w-int(w/2))+shift[:ii, 1].sum())] for jj in np.int32(np.arange(w**2))])
        print(0.2)
        pool.close()
    ...
    with mp.get_context('spawn').Pool(int(n_cpu-1)) as pool:
        rlt = pool.map(partial(tv_l1_subpixel, fixed_img, img), 
