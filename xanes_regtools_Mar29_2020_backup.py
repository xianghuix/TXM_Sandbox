...
#    def set_xanes3D_shift(self, shift, idx=None):
#        """
#        idx is determined as the minimum of diff(shift) so it is index is moved by 1 toward to the left handside;
#        to extract shift, we therefore need to add 1 to idx.
#        """
#        if idx is None:
#            self.best_shift = shift
#        else:
#            if self.shift is None:
#                print('You have not registered the images yet. Quit!')
#                exit()
#            else:
##                self.best_shift = self.shift[idx+1]
#                self.best_shift = self.shift[idx]
#
#    def translate_xanes3D_stack(self, img_stack):
#        if self.best_shift is None:
#            print('You have not set the best shift values yet. Quit!')
#            exit()
#        else:
#            if self.method == 'pc' or self.method == 'mpc':
#                for ii in range(img_stack.shape[0]):
#                    img_stack[ii] = np.real(np.fft.ifftn(fourier_shift(np.fft.fftn(img_stack[ii]), self.best_shift)))[:]
#                    print(ii)
#            elif self.method == 'sr':
#                if self.mode.upper() == 'TRANSLATION':
#                    sr = StackReg(StackReg.TRANSLATION)
...
#
#                for ii in range(img_stack.shape[0]):
#                    img_stack[ii] = sr.transform(img_stack[ii], tmat = self.best_shift)[:]
#
#    def translate_xanes3D_single_img(self, img):
#        if self.best_shift is None:
#            print('You have not set the best shift values yet. Quit!')
#            exit()
#        else:
#            if self.method == 'pc' or self.method == 'mpc':
#                print(self.best_shift)
#                img[:] = np.real(np.fft.ifftn(fourier_shift(np.fft.fftn(img), self.best_shift)))[:]
#            elif self.method == 'sr':
#                print(self.best_shift)
#                if self.mode.upper() == 'TRANSLATION':
#                    sr = StackReg(StackReg.TRANSLATION)
...
#                elif  self.mode.upper() == 'BILINEAR':
#                    sr = StackReg(StackReg.BILINEAR)
#                img[:] = sr.transform(img, tmat = self.best_shift)[:]
#
#    def translate_xanes2D_stack(self, img_stack):
#        if self.shift is None:
#            print('You have not registered images yet. Quit!')
#            exit()
#        else:
#            if self.method == 'pc' or self.method == 'mpc':
#                for ii in range(img_stack.shape[0]):
#                    img_stack[ii] = np.real(np.fft.ifftn(fourier_shift(np.fft.fftn(img_stack[ii]), self.shift[ii]+self.sec_shift[ii])))[:]
#                    print(ii)
#            elif self.method == 'sr':
#                if self.mode.upper() == 'TRANSLATION':
#                    sr = StackReg(StackReg.TRANSLATION)
...
#
#                for ii in range(img_stack.shape[0]):
#                    img_stack[ii] = sr.transform(img_stack[ii], tmat = np.matmul(self.shift[ii], sec_shift[ii]))[:]
#                    print(ii)
