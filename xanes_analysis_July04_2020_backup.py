...
class xanes_analysis():
    ...
    def fit_edge_scipy(self, es, ee, 
        ...
        self.edge_pos_0p5_fit = xm.find_fit_edge_0p5_map_scipy(model, us_eng, self.edge_fit_coef)
    ...
    def calc_direct_edge_0p5(self, peak_es, peak_ee):
    ...
    def fit_whiteline_scipy(self, peak_es, peak_ee, 
        ...
        if self.spec_normalized:
            ...
            #     reshape(self.spectrum.shape[1:])
            # self.edge_pos_0p5_fit = xm.find_fit_edge_0p5_map_scipy(model, us_eng, self.wl_fit_coef).\
            #     reshape(self.spectrum.shape[1:])
        else:
            if flt_spec:
                flt_kernal = np.int32(np.ones(len(self.spectrum.shape)))
