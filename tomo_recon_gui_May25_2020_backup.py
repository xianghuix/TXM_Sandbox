...
FILTER_PARAM_DICT = OrderedDict({'phase retrieval': {0: ['filter', 'filter: filter type used in phase retrieval'],
                                                     1: ['pad', 'pad: boolean, if pad the data before phase retrieval filtering'],
                                                     6: ['pixel_size', 'pixel_size: in cm unit'],
                                                     7: ['dist', 'dist: sample-detector distance in cm'],
                                                     8: ['energy', 'energy: x-ray energy in keV'],
                                                     9: ['alpha', 'alpha: beta/delta, wherr n = (1-delta + i*beta) is x-ray rafractive index in the sample']},

                                 'down sample data': {6: ['level', 'level: data downsampling ratio; level=0.5 downsamples the projection images by a factor 2. Projection angles wont be downsampled']},

                                 'flatting bkg': {6: ['air', 'air: number of pixels on the both sides of projection images where is sample free region. This region will be used to correct background nonunifromness']},

                                 'remove cupping': {6: ['cc', 'cc: constant that is subtracted from the logrithm of the normalized images. This is for correcting the cup-like background in the case when the sample size is much larger than the image view']},

                                 'stripe_removal: vo': {6: ['snr', 'snr: signal-to-noise ratio'],
                                                        7: ['la_size', "la_size: large ring's width in pixel"],
                                                        8:["sm_size", "sm_size: small ring's width in pixel"]},

                                 'stripe_removal: ti': {6: ['nblock', 'nblock: '], 7: ['alpha', 'alphs: ']},

                                 'stripe_removal: sf': {6: ['nblock', 'nblock: ']},

                                 'stripe_removal: fw': {0: ['pad', 'pad: boolean, if padding data before filtering'],
                                                        1: ['wname', 'wname: wavelet name'],
                                                        6: ['level', 'level: how many of level of wavelet transforms'],
                                                        7: ['sigma', 'sigma: sigam of gaussian filter in image Fourier space']},

                                 'denoise: wiener': {0: ['reg', 'reg: '],
                                                     1: ['is_real', 'is_real: '],
                                                     2: ['clip', 'clip: '],
                                                     6: ['psf', 'psf: '],
                                                     7: ['balance', 'balance: ']},

                                 'denoise: unsupervised_wiener': {0: ['reg', 'reg: '],
                                                                  1: ['is_real', 'is_real: '],
                                                                  2: ['clip', 'clip: '],
                                                                  6: ['psf', 'psf: '],
                                                                  7: ['balance', 'balance: ']},

                                 'denoise: denoise_nl_means': {0: ['multichannel', 'multichannel: '],
                                 ...
                                 'denoise_tv_bregman': {0: ['isotrophic', 'isotrophic: '],
                                                        6: ['weight', 'weight: '],
                                                        7: ['max_iter', 'max_iter: '],
                                                        8: ['eps', 'eps: ']},

                                 'denoise_tv_chambolle': {0: ['multichannel', 'multichannel: '],
                                                          6: ['weight', 'weight: '],
                                                          7: ['max_iter', 'max_iter: '],
                                                          8: ['eps', 'eps: ']},

                                 'denoise: denoise_bilateral': {0: ['win_size', 'win_size'],
                                                                1: ['sigma_color', 'sigma_color'],
                                                                2: ['multichannel', 'multichannel: '],
                                                                3: ['mode', 'mode: '],
                                                                6: ['sigma_spatial', 'sigma_spatial: '],
                                                                7: ['bins', 'bins: '],
                                                                8: ['cval', 'cval: ']},

                                 'denoise: denoise_wavelet': {0: ['wavelet', 'wavelet: '],
                                                              1: ['mode', 'mode: '],
                                                              2: ['multichannel', 'multichannel: '],
                                                              3: ['convert2ycbcr', 'convert2ycbcr: '],
                                                              4: ['method', 'method: '],
                                                              6: ['sigma', 'sigma: '],
                                                              7: ['vavelet_levels', 'wavelet_levels: ']}})
...
class tomo_recon_gui():
    def __init__(self, form_sz=[650, 740]):
        ...
        self.tomo_left_filter_list = 'phase retrieval'
    ...
    def set_param_widgests(self, h_set, enable_list, desc_list):
        """
        h_set: list,
        ...
        for h  in h_set:
            h.disabled = True
        for h, d in zip(enable_list, desc_list):
            h.disabled = False
            h.description_tootip = d

    def read_filter_params(self):
        self.filter_param_dict = {}
        flt = FILTER_PARAM_DICT[self.tomo_left_filter_list]
        for idx in flt.keys():
            self.filter_param_dict[flt[idx][0]] = self.flt_phs[idx].value

        # enable_list = OrderedDict(FILTER_PARAM_DICT[self.tomo_left_filter_list]).keys()
        # if self.tomo_left_filter_list == 'phase retrieval':
        #     # enable_list = [0, 1, 6, 7, 8, 9]
        #     self.filter_param_dict = {'filter':self.flt_phs[enable_list[0]].value,
        #                               'pad':self.flt_phs[enable_list[1]].value,
        #                               'pixel_size':self.flt_phs[enable_list[2]].value,
        #                               'dist':self.flt_phs[enable_list[3]].value,
        #                               'energy':self.flt_phs[enable_list[4]].value,
        #                               'alpha':self.flt_phs[enable_list[5]].value}
        # elif self.tomo_left_filter_list == 'down sample data':
        #     # enable_list = [6]
        #     self.filter_param_dict = {'levle':self.flt_phs[enable_list[0]].value}
        # elif self.tomo_left_filter_list == 'flatting bkg':
        #     # enable_list = [6]
        #     self.filter_param_dict = {'air':self.flt_phs[enable_list[0]].value}
        # elif self.tomo_left_filter_list == 'remove cupping':
        #     # enable_list = [6]
        #     self.filter_param_dict = {'cc':self.flt_phs[enable_list[0]].value}
        # elif self.tomo_left_filter_list == 'stripe_removal: vo':
        #     # enable_list = [6, 7, 8]
        #     self.filter_param_dict = {'snr':self.flt_phs[enable_list[0]].value,
        #                               'la_size':self.flt_phs[enable_list[1]].value,
        #                               'sm_size':self.flt_phs[enable_list[2]].value}
        # elif self.tomo_left_filter_list == 'stripe_removal: ti':
        #     # enable_list = [6, 7]
        #     self.filter_param_dict = {'nblock':self.flt_phs[enable_list[0]].value,
        #                               'alpha':self.flt_phs[enable_list[1]].value}
        # elif self.tomo_left_filter_list == 'stripe_removal: sf':
        #     # enable_list = [6]
        #     self.filter_param_dict = {'nblock':self.flt_phs[enable_list[0]].value}
        # elif self.tomo_left_filter_list == 'stripe_removal: fw':
        #     # enable_list = [0, 1, 6, 7]
        #     self.filter_param_dict = {'pad':self.flt_phs[enable_list[0]].value,
        #                               'wname':self.flt_phs[enable_list[1]].value,
        #                               'level':self.flt_phs[enable_list[2]].value,
        #                               'sigma':self.flt_phs[enable_list[3]].value}
        # elif self.tomo_left_filter_list == 'denoise: wiener':
        #     # enable_list = [0, 1, 2, 6, 7]
        #     self.filter_param_dict = {'reg':self.flt_phs[enable_list[0]].value,
        #                               'is_real':self.flt_phs[enable_list[1]].value,
        #                               'clip':self.flt_phs[enable_list[2]].value,
        #                               'psf':self.flt_phs[enable_list[3]].value,
        #                               'balance':self.flt_phs[enable_list[4]].value}
        # elif self.tomo_left_filter_list == 'denoise: unsupervised_wiener':
        #     # enable_list = [0, 1, 2, 6, 7]
        #     self.filter_param_dict = {'reg':self.flt_phs[enable_list[0]].value,
        #                               'is_real':self.flt_phs[enable_list[1]].value,
        #                               'clip':self.flt_phs[enable_list[2]].value,
        #                               'psf':self.flt_phs[enable_list[3]].value,
        #                               'balance':self.flt_phs[enable_list[4]].value}
        # elif self.tomo_left_filter_list == 'denoise: denoise_nl_means':
        #     # enable_list = [0, 1, 6, 7, 8, 9]
        #     self.filter_param_dict = {'multichannel':self.flt_phs[enable_list[0]].value,
        #                               'fast_mode':self.flt_phs[enable_list[1]].value,
        #                               'patch_size':self.flt_phs[enable_list[2]].value,
        #                               'patch_distance':self.flt_phs[enable_list[3]].value,
        #                               'h':self.flt_phs[enable_list[4]].value,
        #                               'sigma':self.flt_phs[enable_list[5]].value}
        # elif self.tomo_left_filter_list == 'denoise_tv_bregman':
        #     # enable_list = [0, 6, 7, 8]
        #     self.filter_param_dict = {'isotropic':self.flt_phs[enable_list[0]].value,
        #                               'weight':self.flt_phs[enable_list[1]].value,
        #                               'max_iter':self.flt_phs[enable_list[2]].value,
        #                               'eps':self.flt_phs[enable_list[3]].value}
        # elif self.tomo_left_filter_list == 'denoise_tv_chambolle':
        #     # enable_list = [0, 6, 7, 8]
        #     self.filter_param_dict = {'multichannel':self.flt_phs[enable_list[0]].value,
        #                               'weight':self.flt_phs[enable_list[1]].value,
        #                               'max_iter':self.flt_phs[enable_list[2]].value,
        #                               'eps':self.flt_phs[enable_list[3]].value}
        # elif self.tomo_left_filter_list == 'denoise: denoise_bilateral':
        #     # enable_list = [0, 1, 2, 3, 6, 7, 8]
        #     self.filter_param_dict = {'win_size':self.flt_phs[enable_list[0]].value,
        #                               'sigma_color':self.flt_phs[enable_list[1]].value,
        #                               'multichannel':self.flt_phs[enable_list[2]].value,
        #                               'mode':self.flt_phs[enable_list[3]].value,
        #                               'sigma_spatial':self.flt_phs[enable_list[4]].value,
        #                               'bins':self.flt_phs[enable_list[5]].value,
        #                               'cval':self.flt_phs[enable_list[6]].value}
        # elif self.tomo_left_filter_list == 'denoise: denoise_wavelet':
        #     # enable_list = [0, 1, 2, 3, 4, 6, 7]
        #     self.filter_param_dict = {'wavelet':self.flt_phs[enable_list[0]].value,
        #                               'mode':self.flt_phs[enable_list[1]].value,
        #                               'multichannel':self.flt_phs[enable_list[2]].value,
        #                               'convert2ycbcr':self.flt_phs[enable_list[3]].value,
        #                               'method':self.flt_phs[enable_list[4]].value,
        #                               'sigma':self.flt_phs[enable_list[5]].value,
        #                               'wavelet_levels':self.flt_phs[enable_list[6]].value}
    ...
    def L0_0_1_0_1_0_0_filter_config_box_left_box_filter_dropdown_change(self, a):
        self.tomo_left_filter_list = a['owner'].value
        if self.tomo_left_filter_list == 'phase retrieval':
            self.filter_param_dict = {''}
        elif self.tomo_left_filter_list == 'down sample data':
            pass
    ...
    def L0_0_1_0_1_0_1_filter_config_box_left_box_move_button_change(self, a):
        ...
        if ((len(self.hs['L[0][0][1][0][1][1][0]_filter_config_right_box_selectmultiple'].options) == 1) &
            ...
            self.hs['L[0][0][1][0][1][1][0]_filter_config_right_box_selectmultiple'].options = [self.tomo_left_filter_list,]
            self.hs['L[0][0][1][0][1][1][0]_filter_config_right_box_selectmultiple'].value = [self.tomo_left_filter_list,]
            self.tomo_right_filter_dict[0] = {self.tomo_left_filter_list:{'p1':None, 'p2':0}}
        else:
            a = list(self.hs['L[0][0][1][0][1][1][0]_filter_config_right_box_selectmultiple'].options)
            a.append(self.tomo_left_filter_list)
