...
class regtools():
        ...
        if self.data_type == '3D_XANES':
            if self.fixed_img_id in range(self.img_id_s, self.img_id_e+1):
                ...
                #     print(self.xanes3D_raw_h5_top_dir)
                    ...
                    #     #     self.eng_list.append(f['/X_eng'][()]*1e3)
            ...
            if self.fixed_img_id in range(self.img_id_s, self.img_id_e+1):
                ...
                #     with h5py.File(self.xanes2D_raw_filename, 'r') as f:
                #         self.eng_list = f['/X_eng'][self.img_id_s:self.img_id_e+1]*1e3
                cnt = 0
                for ii in range(self.img_id_s, self.img_id_e+1):
                    self.eng_dict[str(cnt).zfill(3)] = self.eng_list[cnt]
            ...
            if method.upper() == "SR":
                for key, item in self.shift_chain_dict.items():
                    ...
                    for ii in range(len(item)-1):
                        ...
                        # slioff += int(optional_shift_dict[str(idx)][0]-self.xanes3D_sli_search_half_range)
                ...
                for key, item in self.shift_chain_dict.items():
                    ...
                    for ii in range(len(item)-1):
                        ...
                        # slioff += int(shift_dict[str(idx)][0]-self.xanes3D_sli_search_half_range)
            ...
            if self.mask is not None:
                for ii in range(len(self.alignment_pair_list)):
                    ...
                    for jj in range(self.xanes3D_recon_fixed_sli-self.xanes3D_sli_search_half_range,
                        ...
                        self.shift[ii, jj_id] = masked_register_translation(self.fixed, img[jj_id],
    ...
    def tv_l1_pixel(self, fixed_img, img, y_shift, x_shift):
    ...
    def tv_l2_pixel(self, fixed_img, img, y_shift, x_shift):
        diff_img = fixed_img - np.roll(img, [y_shift, x_shift], axis=[0, 1])        
        tv2 = ((np.diff(diff_img, axis=0, prepend=1))**2+(np.diff(diff_img, axis=1, prepend=1))**2).sum()
        return tv2 
    
    def tv__l1_subpixel(self, fixed_img, img, y_shift, x_shift):
        diff_img = fixed_img - np.real(np.fft.ifftn(fourier_shift(np.fft.fftn(img), [y_shift, x_shift])))
        tv1 = (np.abs(np.diff(diff_img, axis=0, prepend=1))+np.abs(np.diff(diff_img, axis=1, prepend=1))).sum()
        return tv1
    
    def tv__l2_subpixel(self, fixed_img, img, y_shift, x_shift):
        diff_img = fixed_img - np.real(np.fft.ifftn(fourier_shift(np.fft.fftn(img), [y_shift, x_shift])))
        tv2 = ((np.diff(diff_img, axis=0, prepend=1))**2+(np.diff(diff_img, axis=1, prepend=1))**2).sum()
        return tv2 
    
    def multi_resolutin_tv_reg(self, fixed_img, img, levs=4, wz=10, step=0.2):
        sch_config = {}
        sch_config[levs-1] = {'wz':wz, 'step':step}
        ...
        for ii in range(levs-1):
            sch_config[levs-1-ii] = {'wz':wz, 'step':2**ii}
        ...
        for ii in range(levs-1):  
            ...
            with mp.Pool(int(n_cpu/2)) as pool:
                rlt = pool.starmap(self.tv_l1_pixel, [(fixed_img, img, step*(jj//w-int(w/2))+shift[:ii, 0].sum(axis=0), 
                                                       step*(jj%w-int(w/2))+shift[:ii, 1].sum(axis=0)) for jj in np.int32(np.arange(w**2))])
            pool.join()
            pool.close()
            
            for kk in range(w**2):
                tv1_pxl[ii, kk//w, kk%w] = rlt[kk]
            del(rlt)
            gc.collect()
        ...
        with mp.Pool(int(n_cpu/2)) as pool:
            rlt = pool.starmap(self.subpixel, [(fixed_img, img, step*(ii//w-int(w/2))+shift[:(levs-1), 0].sum(axis=0), 
                                                step*(ii%w-int(w/2))+shift[:(levs-1), 1].sum(axis=0)) for ii in np.int32(np.arange(w**2))])
        pool.join()
        pool.close()        
        for jj in range(w**2):
            tv1_pxl[levs-1, jj//w, jj%w] = rlt[jj]
        del(rlt)
        gc.collect()
            
        return tv1_pxl, tv1_pxl_id, shift
