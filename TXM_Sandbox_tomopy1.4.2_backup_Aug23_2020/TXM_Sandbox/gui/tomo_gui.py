...
from ..utils.tomo_recon_tools import FILTERLIST, TOMO_RECON_PARAM_DICT, run_engine
...
class tomo_recon_gui():
    ...
    def build_gui(self):
        ...
        self.hs["L[0][0][0][1][0][1][3][2]_chunk_sz_text"] = widgets.BoundedIntText(value=200, min=0, max=400, description="Chunk Sz", disabled=True)
        layout = {"width":"19%", "height":"90%"}
        self.hs["L[0][0][0][1][0][1][3][2]_chunk_sz_text"].layout = layout
        self.hs["L[0][0][0][1][0][1][3][3]_margin_sz_text"] = widgets.BoundedIntText(value=15, min=0, max=50, description="Margin Sz", disabled=True)
        layout = {"width":"19%", "height":"90%"}
        ...
        # self.hs["L[0][0][0][1][0][1][6][3]_fiji_viewer_checkbox"].observe(self.L0_0_0_1_3_0_0_0_fiji_viewer_checkbox_change, names="value")
        # self.hs["L[0][0][0][1][0][1][6][4]_sli_range_slider"].observe(self.L0_0_0_1_3_0_0_1_sli_range_slider_change, names="value")
        ...
        layout = {"border":"3px solid #FFCC00", "width":f"{self.form_sz[1]-127}px", "height":f"{0.52*(self.form_sz[0]-128)}px"}
        ...
        layout = {"width":"20%"}
        self.hs["L[0][0][0][1][3][0][3][2]_raw_proj_viewer_close_button"] = widgets.Button(description="Close",
        ...
        # self.hs["L[0][0][1][0][0][0][0]_filter_config_title"] = widgets.Text(value="Filter Config", disabled=True)
        ...
        # self.hs["L[0][0][1][0][1][0][0]_reecon_title"] = widgets.Text(value="recon", disabled=True)
    ...
    def L0_0_0_0_1_0_select_raw_h5_top_dir_button_click(self, a):
        ...
        if len(a.files[0]) != 0:
            ...
            for ii in range(6):
                b +=str(t[ii]).zfill(2)+'-'
            self.tomo_cen_dict_fn = os.path.join(self.tomo_raw_data_top_dir, 'recon_cen_list_{}.json'.format(b))
    ...
    def L0_0_0_1_3_0_3_0_raw_proj_slider_change(self, a):
        ...
        if self.load_raw_in_mem:
            if not self.raw_is_in_mem:
                with h5py.File(self.tomo_raw_data_file_template.format(self.tomo_scan_id[0]), 'r') as f:
                    self.raw = ((f['/img_tomo'][:] - f['/img_dark_avg'][:])/
                                (f['/img_bkg_avg'][:] - f['/img_dark_avg'][:])).astype(np.float32)
            ...
            with h5py.File(self.tomo_raw_data_file_template.format(self.tomo_scan_id[0]), 'r') as f:
                self.raw_proj[:] = ((f['/img_tomo'][idx, :] - f['/img_dark_avg'][:])/
                                    (f['/img_bkg_avg'][:] - f['/img_dark_avg'][:])).astype(np.float32)
    ...
    def L0_0_0_1_3_0_0_0_cen_offset_range_slider_change(self, a):
        # if (self.raw_proj_0 is None) & (self.raw_proj_180 is None):
        with h5py.File(self.tomo_raw_data_file_template.format(self.tomo_scan_id[0]), 'r') as f:
            self.raw_proj_0[:] = ((f['/img_tomo'][0, :] - f['/img_dark_avg'][:])/
                               (f['/img_bkg_avg'][:] - f['/img_dark_avg'][:])).astype(np.float32)
            self.raw_proj_0[:] = np.where(self.raw_proj_0<0, 1, self.raw_proj_0)[:]
            self.raw_proj_0[np.isinf(self.raw_proj_0)] = 1
            self.raw_proj_180[:] = ((f['/img_tomo'][-1, :] - f['/img_dark_avg'][:])/
                                 (f['/img_bkg_avg'][:] - f['/img_dark_avg'][:])).astype(np.float32)
    ...
    def L0_0_0_1_3_0_1_1_trial_cen_confirm_button_click(self, a):
        ...
        self.tomo_cen_dict[str(self.tomo_scan_id[0])] = self.trial_cen
        with open(self.tomo_cen_dict_fn, 'w') as f:
            json.dump(self.tomo_cen_dict, f)
    ...
    def L0_0_1_0_1_1_1_reecon_button_change(self, a):
        ...
        if (self.tomo_recon_type == "Trial Center") & (self.recon_finish == 0):
            ...
            self.hs["L[0][0][0][1][3][0][2][0]_vol_sli_slider"].max = int(self.hs["L[0][0][0][1][0][1][3][1]_sli_end_text"].value -
                                                                          ...
                                                                          self.hs["L[0][0][0][1][0][1][3][3]_margin_sz_text"].value)               
