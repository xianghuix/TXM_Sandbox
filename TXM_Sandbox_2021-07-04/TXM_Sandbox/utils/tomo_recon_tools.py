...
TOMO_RECON_PARAM_DICT["recon_config"] = {"recon_type":'Trial Cent',
                                        "use_debug":False,
                                        "use_alt_flat":False,
                                        "use_alt_dark":False,
                                        "use_fake_flat":False,
    ...
    if mean_axis is None:
        if ds_use:
            data = data_down_sampling(reader(fn, dtype='data',
                        ...
                        cfg=cfg), [1, ds_level, ds_level]).astype(np.float32)
    ...
    if rec_type == 'Trial Cent':
        ...
        if flt_param_dict[0]:
            for idx in sorted(flt_param_dict.keys()):
                ...
                if rec_use_debug:
                    save_debug(file_debug_top_dir, '3-filter_name'+str(flt_param_dict[idx].keys())+'_filtered_data.tiff', data)
