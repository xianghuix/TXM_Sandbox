...
class xanes_analysis():
    ...
    def cal_drt_wl_peak_hgt(self, peak_es, peak_ee):
        ...
        for ii in range(1, len(self.spec.shape)):
            eng = eng[:, np.newaxis]
        self.direct_wl_ph = np.squeeze(np.max(self.spec[self.wl_es_idx:self.wl_ee_idx, :], axis=0))

    def cal_wgt_eng(self, eng_s):
        if self.direct_wl_ph[0] is None:
            print('Please calculate the whiteline peak height first. Quit!')
            return 1
        if (self.wl_pos_dir[0] is None) and (self.wl_pos_fit[0] is None):
    ...
    def find_wl(self, es, ee, optimizer='model', ufac=20):
        ...
        if optimizer in 'model':
            self.wl_pos_fit = xm.find_fit_peak(self.model['wl']['model'],
                                               np.array(self.model['wl']['fit_rlt'][0]), us_eng) \
                                  .reshape(self.spec.shape[1:]) + self.model['wl']['eoff']
        elif optimizer == 'direct':
