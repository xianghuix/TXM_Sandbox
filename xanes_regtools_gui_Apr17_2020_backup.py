...
class xanes_regtools_gui():
    def __init__(self, form_sz=[650, 740]):
        ...
        self.xanes2D_config = {"filepath config":{"xanes2D_file_raw_h5_filename":self.xanes2D_file_raw_h5_filename,
                               ...
                               "review registration":{"xanes2D_review_use_existing_reg_reviewed":self.xanes2D_review_read_alignment_option,
                                                      "xanes2D_review_reg_pair":self.xanes2D_review_reg_best_shift,
    ...
    def lock_message_text_boxes(self):
        boxes = ['L[0][2][0][0][1][1]_select_raw_h5_path_text',
                 ...
                 'L[0][2][0][1][4][1]_confirm_config_indices_text',
            ...
            if self.xanes3D_analysis_option in ['Do New Reg', 'Read Config File']:
                if not self.xanes3D_filepath_configured:
                    ...
                    if not self.xanes3D_scan_id_set:
                        boxes = ['L[0][2][0][1]_config_indices_box',
    ...
    def L0_2_2_0_1_0_run_reg_button_click(self, a):
        ...
        reg.set_img_data(self.xanes3D_img)
    ...
    def L0_2_3_1_4_1_analysis_run_button(self, a):
        ...
        code['022'] = "xana = xa.xanes_analysis(imgs, xanes3D_analysis_eng_list, xanes3D_analysis_edge_eng, pre_ee=xanes3D_analysis_pre_edge_e, post_es=xanes3D_analysis_post_edge_s, edge_jump_threshold=xanes3D_analysis_edge_jump_thres, pre_edge_threshold=xanes3D_analysis_edge_offset_thres)"
        code['023'] = ""
        code['024'] = "try:"
        ...
        code['038'] = "        xana.fit_whiteline(xanes3D_analysis_wl_fit_eng_s, xanes3D_analysis_wl_fit_eng_e)"
        code['039'] = "        xana.calc_direct_whiteline(xanes3D_analysis_wl_fit_eng_s, xanes3D_analysis_wl_fit_eng_e)"
        code['040'] = "        g120[ii] = xana.whiteline_pos[:]"
        code['041'] = "        g121[ii] = xana.whiteline_pos_direct[:]"
        code['042'] = "        print(ii)"
        code['043'] = "else:"
        ...
        code['064'] = "        xana.fit_edge_pos(xanes3D_analysis_edge_0p5_fit_s, xanes3D_analysis_edge_0p5_fit_e)"
        code['065'] = "        xana.fit_whiteline(xanes3D_analysis_wl_fit_eng_s, xanes3D_analysis_wl_fit_eng_e, order=3)"
        ...
        code['068'] = "        g120[ii] = np.float32(xana.whiteline_pos)[:]"
        code['069'] = "        g121[ii] = np.float32(xana.whiteline_pos_direct)[:]"
        code['070'] = "        g122[ii] = np.float32(xana.direct_whiteline_ph)[:]"
        ...
        code['075'] = "        g127[ii] = np.float32(xana.edge_eng_pos)[:]"
        code['076'] = ""
        code['077'] = "f.close()"
    ...
    def L0_1_0_0_3_0_select_save_trial_button_click(self, a):
        if self.xanes2D_file_analysis_option == 'Do New Reg':
            ...
            if len(a.files[0]) != 0:
                ...
                for ii in range(6):
                    b +=str(t[ii]).zfill(2)+'-'
                self.xanes2D_file_save_trial_reg_config_filename = os.path.abspath(a.files[0]).split('.')[0]+'_config_'+b.strip('-')+'.json'
                self.hs['L[0][1][0][0][2][0]_select_save_trial_button'].initialdir = os.path.dirname(os.path.abspath(a.files[0]))
                self.hs['L[0][1][0][0][2][0]_select_save_trial_button'].initialfile = os.path.basename(a.files[0])
    ...
    def L0_1_0_0_4_0_confirm_file_path_button_click(self, a):
        if self.xanes2D_file_analysis_option == 'Do New Reg':
            if not self.xanes2D_file_raw_h5_set:
                ...
                for ii in range(6):
                    b +=str(t[ii]).zfill(2)+'-'
                self.xanes2D_file_save_trial_reg_filename = os.path.abspath(self.xanes2D_file_save_trial_reg_filename_template).split('.')[0]+\
                                                            '_'+os.path.basename(self.xanes2D_file_raw_h5_filename).split('.')[0]+'_'+b.strip('-')+'.h5'
                self.xanes2D_file_save_trial_reg_config_filename = os.path.abspath(self.xanes2D_file_save_trial_reg_filename_template).split('.')[0]+\
            ...
            if not self.xanes2D_file_config_file_set:
                ...
                if self.xanes2D_alignment_done:
                    ...
                    self.xanes2D_review_reg_best_match_filename = os.path.splitext(self.xanes3D_save_trial_reg_config_filename)[0].replace('config', 'reg_best_match') + '.json'
    ...
    def L0_1_3_1_4_1_analysis_run_button_click(self, a):
        ...
        code['021'] = "xanes2D_analysis_norm_fitting_order = f['/processed_XANES2D/proc_parameters/normalized_fitting_order'][()]"
        code['022'] = "xana = xa.xanes_analysis(imgs, xanes2D_analysis_eng_list, xanes2D_analysis_edge_eng, pre_ee=xanes2D_analysis_pre_edge_e, post_es=xanes2D_analysis_post_edge_s, edge_jump_threshold=xanes2D_analysis_edge_jump_thres, pre_edge_threshold=xanes2D_analysis_edge_offset_thres)"
        code['023'] = ""
        code['024'] = "try:"
        ...
        code['035'] = "    xana.fit_whiteline(xanes2D_analysis_wl_fit_eng_s, xanes2D_analysis_wl_fit_eng_e)"
        ...
        code['059'] = "    xana.fit_edge_pos(xanes2D_analysis_edge_0p5_fit_s, xanes2D_analysis_edge_0p5_fit_e)"
        code['060'] = "    xana.fit_whiteline(xanes2D_analysis_wl_fit_eng_s, xanes2D_analysis_wl_fit_eng_e, order=3)"
        code['061'] = "    xana.calc_direct_whiteline(xanes2D_analysis_wl_fit_eng_s, xanes2D_analysis_wl_fit_eng_e)"
        code['062'] = "    xana.calc_weighted_eng(xanes2D_analysis_pre_edge_e)"
        code['063'] = "    g120[0] = np.float32(xana.whiteline_pos)[:]"
        code['064'] = "    g121[0] = np.float32(xana.whiteline_pos_direct)[:]"
        code['065'] = "    g122[0] = np.float32(xana.direct_whiteline_ph)[:]"
        code['066'] = "    g123[0] = np.float32(xana.centroid_of_eng)[:]"
        code['067'] = "    g124[0] = np.float32(xana.centroid_of_eng_rel_wl)[:]"
        code['068'] = "    g125[0] = np.float32(xana.weighted_atten)[:]"
        code['069'] = "    g126[0] = np.float32(xana.weighted_eng)[:]"
        code['070'] = "    g127[0] = np.float32(xana.edge_eng_pos)[:]"
        code['071'] = ""
        code['072'] = "f.close()"
