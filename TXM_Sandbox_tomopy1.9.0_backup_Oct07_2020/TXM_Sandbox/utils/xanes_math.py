...
def tv_l1_pixel(fixed_img, img, mask, shift):
...
def tv_l2_pixel(fixed_img, img, mask, shift):
    diff_img = fixed_img - np.roll(img, [shift[0], shift[1]], axis=[0, 1])        
    return (((np.diff(diff_img, axis=0, prepend=1))**2+(np.diff(diff_img, axis=1, prepend=1))**2)*mask).sum() 

def tv_l1_subpixel(fixed_img, img, mask, shift):
    diff_img = fixed_img - np.real(np.fft.ifftn(fourier_shift(np.fft.fftn(img), [shift[0], shift[1]])))
    return ((np.abs(np.diff(diff_img, axis=0, prepend=1))+np.abs(np.diff(diff_img, axis=1, prepend=1)))*mask).sum()

def tv_l2_subpixel(fixed_img, img, mask, shift):
    diff_img = fixed_img - np.real(np.fft.ifftn(fourier_shift(np.fft.fftn(img), [shift[0], shift[1]])))
    return (((np.diff(diff_img, axis=0, prepend=1))**2+(np.diff(diff_img, axis=1, prepend=1))**2)*mask).sum()

# def multi_resolutin_tv_reg(fixed_img, img, levs=4, wz=10, step=0.2):
...
#     with mp.Pool(int(n_cpu/2)) as pool:
#     # with mp.Pool(1) as pool:
#         rlt = pool.starmap(_tv_l1_subpixel, [(fixed_img, img, step*(ii//w-int(w/2))+shift[:(levs-1), 0].sum(), 
#                                                     step*(ii%w-int(w/2))+shift[:(levs-1), 1].sum()) for ii in np.int32(np.arange(w**2))])
#     pool.join()
#     pool.close() 
...
def multi_resolutin_tv_reg(fixed_img, img, levs=4, wz=10, sp_wz=20, sp_step=0.2):
    sch_config = {}
    ...
    mask[int(border_wz/2):-int(border_wz), int(border_wz/2):-int(border_wz)] = 1
    ...
    for ii in range(levs-1): 
        ...
        #     future = pool.map(partial(tv_l1_pixel, fixed_img, img), 
...
def mrtv_mpc_combo_reg(fixed_img, img, us=100, reference_mask=None, overlap_ratio=0.3, sp_wz=20, sp_step=0.2):   
    shift = np.zeros([2, 2])
