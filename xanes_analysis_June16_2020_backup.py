...
"""
    ...
    xanes spectra. The basic functions include:
        1. curve fitting
        2. peak fitting
...
class xanes_analysis():
    ...
    def cal_pre_edge_mean(self):
        ...
        return: ndarray, pre_edge_sd has dimension of spectrum.shape[1:]
        """
        self.pre_edge_mean_map = self.spectrum[self.pre_es_idx:self.pre_ee_idx, :].mean(axis=0)

    def cal_post_edge_mean(self):
        """
        return: ndarray, post_edge_sd has dimension of spectrum.shape[1:]
        """
        self.post_edge_mean_map = self.spectrum[self.post_es_idx:self.post_ee_idx, :].mean(axis=0)
