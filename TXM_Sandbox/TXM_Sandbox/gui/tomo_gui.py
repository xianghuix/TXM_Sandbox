...
class tomo_recon_gui():
    ...
    def build_gui(self):
        ...
        self.hs["L[0][0][0][1][0][1][0][4]_cen_list_button"].on_click(self.L0_0_0_1_0_1_0_4_read_cen_list_button_click)
    ...
    def tomo_compound_logic(self):
        if self.tomo_recon_type == "Trial Center":
            ...
            if self.tomo_raw_data_top_dir_set & self.tomo_recon_path_set:
                ...
                self.hs["L[0][0][0][1][0][1][3][2]_col_start_text"].disabled = False
                self.hs["L[0][0][0][1][0][1][3][3]_col_end_text"].disabled = False
                self.hs["L[0][0][0][1][0][1][0][5]_use_config_checkbox"].value = False
                self.hs["L[0][0][0][1][0][1][0][5]_use_config_checkbox"].disabled = False
    ...
    def L0_0_0_1_0_1_0_0_scan_id_dropdown_change(self, a):
        ...
        if self.data_info:
            if self.hs["L[0][0][0][1][0][1][0][2]_cen_win_left_text"].value >= (self.data_info["img_dim"][2] - self.hs['L[0][0][0][1][0][1][0][3]_cen_win_wz_text'].value - 1):
    ...
    def L0_0_0_1_0_1_0_4_read_cen_list_button_click(self, a):
        if len(a.files[0]) != 0:
    ...
    def L0_0_0_1_3_0_1_1_trial_cen_confirm_button_click(self, a):
        ...
        try:
            ...
            with open(self.tomo_trial_cen_dict_fn, 'w') as f:
                ...
                self.tomo_recon_param_dict["recon_config"]["recon_type"] = "Vol Recon: Multi"
                json.dump(tem, f)
        except:
            tem = {}
            tem[str(self.tomo_recon_param_dict["data_params"]["scan_id"])] = self.tomo_recon_param_dict
    ...
    def L0_0_1_0_1_1_1_recon_button_click(self, a):
        if self.tomo_recon_type == "Trial Center":
            ...
            if self.tomo_use_read_config:
                if os.path.basename(self.tomo_cen_list_file).split('.')[-1] == 'json':
                    tem = self.read_config()
                    if tem is not None:
                        recon_param_dict = tem[list(tem.keys())[0]]
                        ...
                        return
                elif os.path.basename(self.tomo_cen_list_file).split('.')[-1] == 'txt':
                    try:
                        with open(self.tomo_cen_list_file, 'r') as f:
                            idx, cen = f.readline()
                            self.tomo_scan_id = idx
                            self.tomo_rot_cen = cen
                            self.set_rec_param_from_config(recon_param_dict)
                            self.set_widgets_from_rec_param(recon_param_dict)
                            self.set_rec_param_from_widgets()
                            self.set_config_from_rec_param()  
            ...
            if self.tomo_use_read_config and (self.tomo_cen_list_file is not None):
                if os.path.basename(self.tomo_cen_list_file).split('.')[-1] == 'json':
                    ...
                    if tem is not None:
                        for key in tem.keys():
                            ...
                            self.set_widgets_from_rec_param(recon_param_dict)
                            print('2:', self.tomo_recon_param_dict)
                            self.hs["L[0][0][0][1][0][1][0][5]_use_config_checkbox"].value = True
                            self.hs["L[0][0][0][1][0][1][0][5]_use_config_checkbox"].disabled = True
                            self.set_rec_param_from_widgets()
                            print('3:', self.tomo_recon_param_dict)
                            self.set_config_from_rec_param()
                            print('4:', self.tomo_recon_param_dict)
                            self.recon_finish = run_engine(**self.tomo_recon_param_dict)
                            if self.recon_finish == 0:
                    ...
                    try:
                        with open(self.tomo_cen_list_file, 'r') as f:
                            idx_cen = np.array(f.readlines())
                            for ii in range(idx_cen.shape[0]):
                                self.tomo_scan_id = idx_cen[ii, 0]
                                self.tomo_rot_cen = idx_cen[ii, 1]
                                self.set_rec_param_from_config(recon_param_dict)
                                self.set_widgets_from_rec_param(recon_param_dict)
                                self.hs["L[0][0][0][1][0][1][0][5]_use_config_checkbox"].value = True
                                self.hs["L[0][0][0][1][0][1][0][5]_use_config_checkbox"].disabled = True
                ...
                print('A configuration file is needed for "Vol Recon: Multi".')
