...
class tomo_recon_gui():
    ...
    def build_gui(self):
        ...
        layout = {"width":"20%"}
        self.hs["L[0][0][0][1][0][1][5][5]_auto_thres_text"] = widgets.BoundedFloatText(value=0.1,
                                                                                        min=-1,
                                                                                        max=1,
        ...
        self.hs["L[0][0][1][0][1][1][1]_recon_button"].on_click(self.L0_0_1_0_1_1_1_recon_button_change)
    ...
    def read_param_from_config_dict(self, recon_param_dict):        
    ...
    def set_widgets(self, recon_param_dict):        
        ...
        if len(a.files[0]) != 0:
            ...
            if self.tomo_use_read_config & (self.tomo_recon_type=="Vol Recon: Single"):
                with open(self.tomo_cen_list_file, 'r') as f:
                    ...
                    self.read_param_from_config_dict(recon_param_dict)
                    self.set_widgets(recon_param_dict)
        else:
            self.tomo_cen_list_file = None
        self.tomo_data_configured = False
        self.recon_finish = -1
    ...
    def L0_0_1_0_0_1_1_2_auto_ref_fn_button_change(self, a):
        if len(a.files[0]) != 0:
            ...
            self.wedge_eva_data, _, _, _ = read_data(self.tomo_wedge_ang_auto_det_ref_fn, self.global_h.io_tomo_cfg, 
                                                      sli_start=self.tomo_sli_s, sli_end=self.tomo_sli_e,
                                                      col_start=self.tomo_col_s, col_end=self.tomo_col_e,
                                                      flat_name=None, dark_name=None,
                                                      use_fake_flat=False, use_fake_dark=False,
                                                      fake_flat_val=1e4, fake_dark_val=100,
                                                      ds_use=False, ds_level=1.0, mean_axis=2)
    ...
    def L0_0_0_1_3_0_3_2_raw_proj_viewer_close_button_click(self, a):
        ...
        if viewer_state is not None:
            ...
            self.hs["L[0][0][0][1][0][1][3][0]_sli_start_text"].value = int(x[0])
            self.hs["L[0][0][0][1][0][1][3][1]_sli_end_text"].value = int(x[2] - x[0])
            self.hs["L[0][0][0][1][0][1][3][2]_col_start_text"].value = int(y[0])
            self.hs["L[0][0][0][1][0][1][3][3]_col_end_text"].value = int(y[2] - y[0])
            self.hs["L[0][0][0][1][0][1][0][2]_cen_win_left_text"].value = int((y[2] - y[0])/2)
    ...
    def L0_0_0_1_3_0_1_1_trial_cen_confirm_button_click(self, a):
        ...
        try:
            with open(self.tomo_trial_cen_dict_fn, 'w+') as f:
                ...
                json.dump(self.tomo_recon_param_dict, f)
        except:
            with open(self.tomo_trial_cen_dict_fn, 'w') as f:
                json.dump(self.tomo_recon_param_dict, f)
    ...
    def L0_0_1_0_1_1_1_recon_button_change(self, a):
