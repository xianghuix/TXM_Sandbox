...
def get_dim_ang_range_single_sli(sino, thres=500., block_view_at=90):
    """
    input:
        sino: ndarray
...
def get_dim_ang_range_range_sli(fn, sli_start, sli_end=None, dataset_path='/img_tomo', thres=500, block_view_at=90):
    """
    inputs:
        fn: str,
    ...
    for ii in range(sli_start, sli_end):
        bad_angs[ii] = get_dim_ang_range_single_sli(f[dataset_path][:, ii, :], thres=thres, block_view_at=block_view_at)
    f.close()
    return bad_angs

def get_file(raw_data_top_dir, scan_id):
...
def read_data(fn, sli_start=0, sli_end=20,
    ...
    if ds_use:
        ...
        try:
           ...
           return None
        data = data_down_sampling(arr[:,sli_start:sli_end,:], [1, ds_level, ds_level]).astype(np.float32)
        f.close()
        ...
        if use_fake_flat:
            ...
            try:
                ...
                return None
            white = data_down_sampling(arr[:,sli_start:sli_end,:], [1, ds_level, ds_level]).astype(np.float32)
            f.close()
        ...
        if use_fake_dark:
            ...
            try:
                ...
                return None
            dark = data_down_sampling(arr[1:9,sli_start:sli_end,:], [1, ds_level, ds_level]).astype(np.float32)
            f.close()
        ...
        try:
            ...
            return None
        data = arr[:,sli_start:sli_end,:].astype(np.float32)
        f.close()
        ...
        if use_fake_flat:
            ...
            try:
                ...
                return None
            white = arr[:,sli_start:sli_end,:].astype(np.float32)
            f.close()
        ...
        if use_fake_dark:
            ...
            try:
                ...
                return None
            dark = arr[:,sli_start:sli_end,:].astype(np.float32)
            f.close()
...
def run_engine(**kwargs):
    ...
    if rec_type == 'Trial Center':
        ...
        data, white, dark, theta = read_data(file_raw_fn, sli_start=data_sli_s,sli_end=data_sli_s+20,
        ...
        if rec_is_wedge:
            if rec_use_wedge_ang_auto_det:
                bad_angs = get_dim_ang_range_range_sli(file_wedge_ang_auto_det_ref_fn,
                                                       data_sli_s, sli_end=data_sli_s+20,
                                                       thres=data_wedge_ang_auto_det_thres,
                                                       block_view_at=data_wedge_blankat)
            else:
        ...
        for idx, cen in zip(idx_list, cen_list):
            file_raw_fn, file_recon_template = get_file(file_raw_data_top_dir, idx)
            ...
            for ii in range(num_chunk):
                try:
                    ...
                    if (sli_end - sli_start) <= data_margin:
                        break
                    data, white, dark, theta = read_data(file_raw_fn, sli_start=sli_start, sli_end=sli_end,
                                                         flat_name=file_alt_flat_fn, dark_name=file_alt_dark_fn,
                                                         use_fake_flat=rec_use_fake_flat, use_fake_dark=rec_use_fake_dark,
                                                         fake_flat_val=data_fake_flat_val, fake_dark_val=data_fake_dark_val,
                                                         ds_use=rec_use_ds, ds_level=data_ds_level)

                    # if None in [data, white, dark, theta]:
