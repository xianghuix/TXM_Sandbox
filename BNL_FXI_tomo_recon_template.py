...
def getFiles(user_top_dir, scan_id):
...
def manualFindCenter(center_shift,center_shift_w,**kwargs):
    ...
    debug = kwargs['ExplicitParams']['debug']
    ...
    data,white,dark,theta = dataStandardReader(filename,sliceStart=sliceStart,sliceEnd=sliceEnd, 
                                               ...
                                               fake_flat=fake_flat, fake_dark=fake_dark, fake_flat_val=fake_flat_val, fake_dark_val=fake_dark_val)
...
#    # remove zingers (pixels with abnormal counts)
    ...
    if debug is True:
        debug_dir = os.path.join(Path(os.path.dirname(data_center_path)).parents[0], 'debug')
        if not os.path.isdir(debug_dir):
            os.mkdir(debug_dir)
        tifffile.imsave(os.path.join(debug_dir, 'normalized_data.tiff'), data) 
...
#    data = filters.gaussian_laplace(data,5)
    ...
    if debug is True:
        ...
        if not os.path.isdir(debug_dir):
            os.mkdir(debug_dir)
        tifffile.imsave(os.path.join(debug_dir, 'filtered_data.tiff'), data) 
...
##### This is the only section which you need to edit
    ...
    user_top_dir = "/media/FXIUSB1/2019_03/FengLin"
    # user_top_dir = "/media/FXIUSB1/BNL_FXI_2019_03/"
...
#    scan_idx = [24567, 24568, 24569, 24570, 24571, 24572, 24573, 24574, 24575, 24576, 24577, 24578]
    ...
    #offset = 250         # set to None if you want to reconstruct the whole volume
    ...
    # wiener_denoiseParams = {'psfw': 3,
    #                         'reg': 0.3}

#    pr_filter = 'paganin'   #'bronnikov'
#    pr_z =15.0                   # sample-detector distance in cm
#    pr_eng = 35.0              # x-ray energy in keV
#    pr_pxl =0.65e-4             # detector pixel size in cm
#    pr_rat =1e-4               # you need to adjust this parameter to balance sharpness and contrast in your reconstructed slice images
#                            # smaller 'rat make contrast better but in price of sharpness;
    retrieve_phaseParams = {'filter': 'bronnikov',
                            'pixel_size': 0.65e-4,
                            'dist': 15,
                            'energy': 35.0,
                            'alpha': 1e-2,
    ...
    remove_stripe_fwParams = {'level': 6,
                              'wname': 'db5',
                              'sigma': 2,
                              'pad':True}
...
############################### user input section -- End ###############################
    ...
    for jj in range(len(scan_idx)):
        if manualCenter == True:
            if len(scan_idx) == 1:
                data_files, output_files = getFiles(user_top_dir, scan_idx[0])
                if data_files == 0:
                    print ('!!!!! Error !!!!!')
            ...
            data_files, output_files = getFiles(user_top_dir, scan_idx[jj])
            if data_files == 0:
                print ('!!!!! Error !!!!!')
                print ('There is no given file in the path. Reconstructon is aborted.')
                exit()
