...
class xanes_analysis():
    ...
    def fit_whiteline_poly(self, peak_es, peak_ee, 
        ...
        if self.spec_normalized:
            ...
            for ii in range(1, len(self.wl_fit_coef.shape)):
                ...
                eng = eng[:, np.newaxis]
            self.wl_pos = xm.index_lookup(xm.find_fit_peak_map_poly(self.wl_fit_coef, us_idx), 
                                          eng, ufac=ufac)
            self.edge_pos = xm.index_lookup(xm.find_deriv_peak_map_poly(self.wl_fit_coef, us_idx), 
                                          eng, ufac=ufac)
            self.edge_pos_0p5 = xm.index_lookup(xm.find_fit_edge_0p5_map_poly(self.wl_fit_coef, us_idx), 
                                          eng, ufac=ufac)
        else:
            if flt_spec:
                flt_kernal = np.int32(np.ones(len(self.spectrum.shape)))
    ...
    def fit_whiteline_scipy(self, peak_es, peak_ee, 
        ...
        if self.spec_normalized:
            ...
            self.wl_pos = xm.find_fit_peak_map_scipy(self.wl_model, us_eng, 
                                                     self.wl_fit_coef).\
                reshape(self.spectrum.shape[1:])
            self.edge_pos = xm.find_deriv_peak_map_scipy(model, us_eng, self.wl_fit_coef).\
                reshape(self.spectrum.shape[1:])
            self.edge_pos_0p5 = xm.find_fit_edge_0p5_map_scipy(model, us_eng, self.wl_fit_coef).\
                reshape(self.spectrum.shape[1:])
        else:
            if flt_spec:
                flt_kernal = np.int32(np.ones(len(self.spectrum.shape)))
    ...
    def fit_edge_poly(self, es, ee, 
        ...
        self.edge_pos_0p5 = xm.index_lookup(xm.find_fit_edge_0p5_map_poly(self.edge_fit_coef, us_idx), 
                                            eng, ufac=ufac)
        self.edge_pos = xm.index_lookup(xm.find_fit_peak_map_poly(self.edge_fit_coef, us_idx), 
                                        eng, ufac=ufac)
    ...
    def fit_edge_scipy(self, es, ee, 
        ...
        self.edge_pos_0p5_ = xm.find_fit_edge_0p5_map_scipy(model, us_eng, self.edge_fit_coef)
        self.edge_pos = xm.find_deriv_peak_map_scipy(model, us_eng, self.edge_fit_coef)
        # self.edge_pos_0p5_us_idx = xm.find_fit_edge_0p5_map_scipy(model, us_idx, self.edge_fit_coef)
