...
class xanes_regtools_gui():
    def __init__(self, form_sz=[650, 740]):
        ...
        self.xanes3D_read_alignment_option = False

        self.xanes3D_fiji_viewer_ip = None
        ...
        self.xanes3D_read_alignment_option = False
        self.xanes3D_reg_file_ready = False
        ...
        self.xanes3D_alignment_file_read = False
    ...
    def boxes_logic(self):
        if not self.xanes3D_filepath_configured:
        ...
        elif ((self.xanes3D_filepath_configured & self.xanes3D_reg_file_ready) 
            ...
            boxes = ['review_reg_results_box',
                     'align_recon_box']
            self.enable_disable_boxes(boxes, disabled=False, level=-1)
        elif ((self.xanes3D_filepath_configured & (not self.xanes3D_read_alignment_option))
            ...
            boxes = ['config_reg_params_box']
            self.enable_disable_boxes(boxes, disabled=False, level=-1)
            if not self.hs['L[0][2][1][1][2][0]_use_mask_checkbox'].value:
    ...
    def gui_layout(self):
        ...
        self.hs['L[0][2][0][1][3][0]_fiji_show_checkbox'].observe(self.L0_2_0_1_3_0_fiji_show_checkbox_change, names='value')
        ...
        ## ## ## ## ## run reg & status
        layout = {'border':'3px solid #FFCC00', 'width':f'{self.form_sz[1]-98}px', 'height':f'{0.2*0.4*(self.form_sz[0]-128)}px'}          
        self.hs['L[0][2][2][2][1]_align_recon_comfirm_box'] = widgets.HBox()
        self.hs['L[0][2][2][2][1]_align_recon_comfirm_box'].layout = layout 
        layout = {'top': '5px','width':'70%', 'height':'100%'}
        ...
        self.hs['L[0][2][2][2][1]_align_recon_comfirm_box'].children = self.get_handles('L[0][2][2][2][1]_align_recon_comfirm_box', -1)
    ...
    def L0_2_0_0_1_0_select_raw_h5_path_button_click(self, a):
        if len(a.files[0]) != 0:
            ...
            a = glob.glob(os.path.join(self.xanes3D_raw_3D_h5_top_dir, 'fly*.h5'))
            self.xanes3D_available_raw_ids = sorted([int(ii.split('.')[0].split('_')[-1]) for ii in a])
    ...
    def L0_2_0_0_2_0_select_recon_path_button_click(self, b):
        if len(b.files[0]) != 0:
            self.xanes3D_recon_3D_top_dir = os.path.abspath(b.files[0])
            ...
            a = glob.glob(os.path.join(self.xanes3D_recon_3D_top_dir, 'recon_fly*'))
            self.xanes3D_available_recon_ids = sorted([int(ii.split('.')[0].split('_')[-1]) for ii in a])
    ...
    def L0_2_0_0_3_0_select_save_trial_button_click(self, c):
        if len(c.files[0]) != 0:
    ...
    def L0_2_0_0_4_2_review_reg_result_checkbox_change(self, a):
        ...
        if self.hs['L[0][2][0][0][4][2]_review_reg_result_checkbox'].value:
            self.xanes3D_reg_file_ready = True
        else:
            self.xanes3D_reg_file_ready = False
        self.boxes_logic()
            
    def L0_2_0_0_4_0_confirm_file_path_button_click(self, a):
        if self.xanes3D_reg_file_ready:
        ...
        if os.path.exists(self.xanes3D_raw_3D_h5_temp.format(a['owner'].value)):
            if os.path.exists(self.xanes3D_recon_3D_dir_temp.format(a['owner'].value)):
                if (a['owner'].value in self.xanes3D_available_raw_ids) and (a['owner'].value in self.xanes3D_available_recon_ids):
                    ...
                    boxes = ['fixed_id_box']
                    self.enable_disable_boxes(boxes, disabled=False)
                    boxes = ['fiji_show_checkbox',
                             'config_indices_confirm_box',
                             'reg_setting_form',
                             'reg&review_form']
                    self.enable_disable_boxes(boxes, disabled=True)
                    fn = sorted(glob.glob(os.path.join(self.xanes3D_recon_3D_dir_temp.format(a['owner'].value), '*.tiff')))[0]
                    try:
                    ...
                    self.hs['L[0][2][0][1][4][1]_confirm_config_indices_text'].value = 'Your specified scan id is not available in the specified directory ...'
                    boxes = ['fixed_id_box',
                             'fiji_show_checkbox',
                             'config_indices_confirm_box',
                             'reg_setting_form',
    ...
    def L0_2_0_1_2_0_fixed_scan_id_slider_change(self, a):
        ...
        a = glob.glob(os.path.join(self.xanes3D_recon_3D_dir_temp.format(self.xanes3D_fixed_scan_id),
                                   f'recon_fly_scan_id_{self.xanes3D_fixed_scan_id}_*.tiff'))
        self.xanes3D_available_recon_file_ids = sorted([int(ii.split('.')[0].split('_')[-1]) for ii in a])
        ...
        if self.hs['L[0][2][0][1][3]_fiji_box'].disabled:
            boxes = ['fiji_box']
            self.enable_disable_boxes(boxes, disabled=False, level=-1)
    
    def L0_2_0_1_2_1_fixed_sli_id_slider_change(self, a):
        if self.hs['L[0][2][0][1][3][0]_fiji_show_checkbox'].value:
            self.xanes3D_fiji_viewer_ip.setSlice(a['owner'].value-a['owner'].min+1)
    ...
    def L0_2_0_1_3_0_fiji_show_checkbox_change(self, a):
        if a['owner'].value:
            ...
            self.xanes3D_fiji_viewer_ip = WindowManager.getCurrentImage()
            self.xanes3D_fiji_viewer_ip.setSlice(self.xanes3D_fixed_sli_id - self.hs['L[0][2][0][1][2][1]_fixed_sli_id_slider'].min + 1) 
            self.hs['L[0][2][0][1][3][1]_fiji_close_button'].disabled=False  
            self.hs['L[0][2][0][1][4][0]_confirm_config_indices_button'].disabled = False              
        else:
            self.xanes3D_fiji_viewer_ip.close()
            self.xanes3D_fiji_viewer_ip = None
            boxes = ['reg_setting_form',
                     'reg&review_form',
                     'confirm_config_indices_button']
            self.enable_disable_boxes(boxes, disabled=True, level=-1)
    
    def L0_2_0_1_3_1_fiji_close_button_click(self, a):
        if self.xanes3D_fiji_viewer_ip is not None:
            if self.hs['L[0][2][0][1][3][0]_fiji_show_checkbox'].value:
                self.hs['L[0][2][0][1][3][0]_fiji_show_checkbox'].value = False
    ...
    def L0_2_0_1_4_0_confirm_config_indices_button_click(self, a):
        if self.xanes3D_fiji_viewer_ip is not None:
            ...
            self.hs['L[0][2][1][0][1][0]_3D_roi_x_slider'].max = self.xanes3D_fiji_viewer_ip.width
            self.hs['L[0][2][1][0][1][0]_3D_roi_x_slider'].min = 0
            self.hs['L[0][2][1][0][1][1]_3D_roi_y_slider'].max = self.xanes3D_fiji_viewer_ip.height
    ...
    def L0_2_1_0_1_0_3D_roi_x_slider_change(self, a):
        if self.hs['L[0][2][1][1]_config_reg_params_box'].disabled is False:
            self.hs['L[0][2][1][1]_config_reg_params_box'].disabled = True
            self.hs['L[0][2][1][0][2][0]_confirm_roi_text'].value = 'Please confirm after ROI is set'
        self.xanes3D_fiji_viewer_ip.setRoi(self.hs['L[0][2][1][0][1][0]_3D_roi_x_slider'].value[0],
    ...
    def L0_2_1_0_1_2_3D_roi_z_slider_change(self, a):
        if self.hs['L[0][2][1][1]_config_reg_params_box'].disabled is False:
            self.hs['L[0][2][1][1]_config_reg_params_box'].disabled = True
            self.hs['L[0][2][1][0][2][0]_confirm_roi_text'].value = 'Please confirm after ROI is set'
        self.xanes3D_fiji_viewer_ip.setSlice(self.hs['L[0][2][1][0][1][2]_3D_roi_z_slider'].value[0]-self.hs['L[0][2][1][0][1][2]_3D_roi_z_slider'].min+1)
    ...
    def L0_2_1_1_1_0_fiji_checkbox_change(self, a):
        pass
    ...
    def L0_2_1_1_4_0_ref_mode_dropdown_change(self, a):
        pass
    
    def L0_2_1_1_5_0_confirm_reg_params_button_click(self, a):
        pass
    ...
    def L0_2_2_2_1_1_align_button_click(self, a):
        pass
...
#         self.mask_viewer_ip.setImage(ij.convert().convert(ij.dataset().create(ij.py.to_java(self.xanes3D_img*self.mask)), ImagePlusClass))
...
#             self.xanes3D_read_alignment_option = True
#         else:
...
#         if self.xanes3D_read_alignment_option:
