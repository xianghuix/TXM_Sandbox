...
from .gui_components import (SelectFilesButton, get_handles, msgbox,
...
from ..utils.tomo_recon_tools import (FILTERLIST, TOMO_RECON_PARAM_DICT,
                                      run_engine, read_data, normalize)
...
FILTER_PARAM_DICT = OrderedDict(
    {"phase retrieval": {0: ["filter", ["paganin", "bronnikov"], "filter: filter type used in phase retrieval"],
     ...
     "stripe_removal: fw": {0: ["pad", ["True", "False"], "pad: boolean, if padding data before filtering"],
                            1: ["wname", ["db5", "db1", "db2", "db3", "sym2", "sym6", "haar", "gaus1", "gaus2", "gaus3",
     ...
     "denoise: unsupervised_wiener": {0: ["reg", ["None"],
     ...
     "denoise: denoise_tv_bregman": {0: ["multichannel", ["False", "True"],
     ...
     "denoise: denoise_tv_chambolle": {0: ["multichannel", ["False", "True"],
                                           "multichannel: Apply total-variation denoising separately for each channel. This option should be true for color images, otherwise the denoising is also applied in the channels dimension"],
                                       6: ["weight", 0.1,
     ...
     "denoise: denoise_bilateral": {0: ["win_size", ["None"],
                                    ...
                                    2: ["multichannel", ["False", "True"],
                                        "multichannel: Whether the last axis of the image is to be interpreted as multiple channels or another spatial dimension"],
                                    3: ["mode", ["constant", "edge", "symmetric", "reflect", "wrap"],
     ...
     "denoise: denoise_wavelet": {
         0: ["wavelet", ["db1", "db2", "db3", "db5", "sym2", "sym6", "haar", "gaus1", "gaus2", "gaus3", "gaus4"],
    ...
    0: ["filter_name", ["parzen", 'shepp', 'cosine', 'hann', 'hamming', 'ramlak', 'butterworth', 'none'],
        "filter_name: filter that is used in frequency space", str]},
                              "sirt": {3: ["num_gridx", 1280,
                                           "num_gridx: number of the reconstructed slice image along x direction", int],
                                       4: ["num_gridy", 1280,
                                           "num_gridy: number of the reconstructed slice image along y direction", int],
                                       5: ["num_iter", 10, "num_iter: number of reconstruction iterations", int]},
                              "tv": {3: ["num_gridx", 1280,
                                         "num_gridx: number of the reconstructed slice image along x direction", int],
                                     4: ["num_gridy", 1280,
                                         "num_gridy: number of the reconstructed slice image along y direction", int],
                              ...
                              "mlem": {3: ["num_gridx", 1280,
                                           "num_gridx: number of the reconstructed slice image along x direction", int],
                                       4: ["num_gridy", 1280,
                                           "num_gridy: number of the reconstructed slice image along y direction", int],
                                       5: ["num_iter", 10, "num_iter: number of reconstruction iterations", int]},
                              "astra": {0: ["method", ["EM_CUDA"], "method: astra reconstruction methods", str],
                                        1: ["proj_type", ["cuda"],
                                            "proj_type: projection calculation options used in astra", str],
                                        2: ["extra_options", ["MinConstraint"],
...
class tomo_recon_gui():
    def __init__(self, parent_h, form_sz=[650, 740]):
        ...
        self.tomo_recon_external_command_name = os.path.join(os.path.abspath(os.path.curdir),
                                                             'tomo_recon_external_command.py')
    ...
    def build_gui(self):
        ...
        self.hs["Filter&Recon tab"].layout = layout
        ...
        self.hs["SelRawH5TopDir text"] = widgets.Text(value="Choose raw h5 top dir ...",
                                                      description="", disabled=True)
        layout = {"width": "66%", "display": "inline_flex"}
        self.hs["SelRawH5TopDir text"].layout = layout
        self.hs["SelRawH5TopDir btn"] = SelectFilesButton(option="askdirectory",
                                                          text_h=self.hs["SelRawH5TopDir text"])
        self.hs["SelRawH5TopDir btn"].description = "Raw Top Dir"
        self.hs[
            "SelRawH5TopDir btn"].description_tooltip = "Select the top directory in which the raw h5 files are located."
        layout = {"width": "15%"}
        self.hs["SelRawH5TopDir btn"].layout = layout
        self.hs["SelRawH5TopDir btn"].on_click(self.L0_0_0_0_1_0_select_raw_h5_top_dir_btn_click)
        self.hs["SelRaw box"].children = [self.hs["SelRawH5TopDir text"],
        ...
        self.hs["SelSavReconDir btn"] = SelectFilesButton(option="askdirectory",
                                                          text_h=self.hs["SelSavReconDir text"])
        ...
        self.hs["SelSavReconDir btn"].on_click(self.L0_0_0_0_2_0_select_save_recon_dir_btn_click)
        self.hs["SelSavRecon box"].children = [self.hs["SelSavReconDir text"],
        ...
        self.hs["SelSavDebugDir text"] = widgets.Text(value="Debug is disabled...",
                                                      description="", disabled=True)
        layout = {"width": "66%", "display": "inline_flex"}
        self.hs["SelSavDebugDir text"].layout = layout
        self.hs["SelSavDebugDir btn"] = SelectFilesButton(option="askdirectory",
                                                          text_h=self.hs["SelSavDebugDir text"])
        ...
        self.hs["SavDebug chbx"] = widgets.Checkbox(value=False, description="Save Debug",
                                                    disabled=False, indent=False)
        layout = {"left": "1%", "width": "13%", "display": "inline_flex"}
        self.hs["SavDebug chbx"].layout = layout
        self.hs["SelSavDebugDir btn"].on_click(self.L0_0_0_0_4_0_select_save_debug_dir_btn_click)
        self.hs["SavDebug chbx"].observe(self.L0_0_0_0_4_2_save_debug_checkbox_change, names="value")
        self.hs["SelSavDebug box"].children = [self.hs["SelSavDebugDir text"],
        ...
        self.hs["SelFile&PathCfm text"] = widgets.Text(value="After setting directories, confirm to proceed ...",
                                                       description="", disabled=True)
        layout = {"width": "66%"}
        self.hs["SelFile&PathCfm text"].layout = layout
        self.hs["SelFile&PathCfm btn"] = widgets.Button(description="Confirm",
                                                        tooltip="Confirm: Confirm after you finish file configuration")
        self.hs["SelFile&PathCfm btn"].style.button_color = "darkviolet"
        self.hs["SelFile&PathCfm btn"].on_click(self.L0_0_0_0_5_0_confirm_file_path_btn_click)
        layout = {"width": "15%"}
        self.hs["SelFile&PathCfm btn"].layout = layout

        self.hs["File&PathOptn drpdn"] = widgets.Dropdown(value="Trial Cent",
        ...
        self.hs["File&PathOptn drpdn"].observe(self.L0_0_0_0_5_2_recon_option_dropdown_change, names="value")
        self.hs["SelFile&PathCfm box"].children = [self.hs["SelFile&PathCfm text"],
                                                   self.hs["SelFile&PathCfm btn"],
                                                   self.hs["File&PathOptn drpdn"]]

        self.hs["SelFile&Path box"].children = [self.hs["SelFile&PathTitle box"],
                                                self.hs["SelRaw box"],
        ...
        layout = {"border": "3px solid #8855AA", "width": f"{self.form_sz[1] - 94}px",
                  "height": f"{0.56 * (self.form_sz[0] - 110)}px"}
        self.hs["Data tab"] = widgets.Tab()
        self.hs["Data tab"].layout = layout

        ## ## ## ## ## define sub-tabs in data_tab -- start
        layout = {"border": "3px solid #FFCC00", "left": "-8px", "width": f"{self.form_sz[1] - 114}px",
                  "top": "-8px", "height": f"{0.49 * (self.form_sz[0] - 116)}px"}
        ...
        self.hs["Data tab"].children = [self.hs["DataConfig tab"],
        ...
        layout = {"border": "3px solid #FFCC00", "width": f"{self.form_sz[1] - 120}px",
        ...
        self.hs["ScanId drpdn"] = widgets.Dropdown(value=0, options=[0],
                                                   description="Scan id", disabled=True)
        layout = {"width": "19%"}
        self.hs["ScanId drpdn"].layout = layout
        self.hs["RotCen text"] = widgets.BoundedFloatText(value=1280.0, min=0, max=2500,
                                                          description="Center", disabled=True)
        layout = {"width": "19%"}
        self.hs["RotCen text"].layout = layout
        self.hs["CenWinLeft text"] = widgets.BoundedIntText(value=1240, min=0, max=2500,
                                                            description="Cen Win L", disabled=True)
        layout = {"width": "19%"}
        self.hs["CenWinLeft text"].layout = layout
        self.hs["CenWinWz text"] = widgets.BoundedIntText(value=80, min=1, max=200,
                                                          description="Cen Win W", disabled=True)
        layout = {"width": "19%"}
        ...
        self.hs["ScanId drpdn"].observe(self.L0_0_0_1_0_1_0_0_scan_id_dropdown_change, names="value")
        self.hs["RotCen text"].observe(self.L0_0_0_1_0_1_0_1_rot_cen_text_change, names="value")
        self.hs["CenWinLeft text"].observe(self.L0_0_0_1_0_1_0_2_cen_win_left_text_change, names="value")
        self.hs["CenWinWz text"].observe(self.L0_0_0_1_0_1_0_3_cen_win_wz_text_change, names="value")
        self.hs["CenList btn"].on_click(self.L0_0_0_1_0_1_0_4_read_cen_list_btn_click)
        self.hs["UseConfig chbx"].observe(self.L0_0_0_1_0_1_0_5_use_config_checkbox_change, names='value')
        ...
        self.hs["RoiSliStart text"] = widgets.BoundedIntText(value=1280, min=0, max=2100,
                                                             description="Sli Start", disabled=True)
        layout = {"width": "19%"}
        self.hs["RoiSliStart text"].layout = layout
        self.hs["RoiSliEnd text"] = widgets.BoundedIntText(value=1300, min=0, max=2200,
                                                           description="Sli End", disabled=True)
        layout = {"width": "19%"}
        self.hs["RoiSliEnd text"].layout = layout
        self.hs["RoiColStart text"] = widgets.BoundedIntText(value=0, min=0, max=400,
                                                             description="Col Start", disabled=True)
        layout = {"width": "19%"}
        self.hs["RoiColStart text"].layout = layout
        self.hs["RoiColEnd text"] = widgets.BoundedIntText(value=10, min=0, max=400,
                                                           description="Col_End", disabled=True)
        layout = {"width": "19%"}
        self.hs["RoiColEnd text"].layout = layout
        self.hs["DnSampRat text"] = widgets.BoundedFloatText(value=1, description="Down Sam R",
                                                             min=0.1, max=1.0, step=0.1, disabled=True)
        layout = {"width": "19%"}
        self.hs["DnSampRat text"].layout = layout

        self.hs["RoiSliStart text"].observe(self.L0_0_0_1_0_1_3_0_sli_start_text_change, names="value")
        self.hs["RoiSliEnd text"].observe(self.L0_0_0_1_0_1_3_1_sli_end_text_change, names="value")
        self.hs["RoiColStart text"].observe(self.L0_0_0_1_0_1_3_2_col_start_text_change, names="value")
        self.hs["RoiColEnd text"].observe(self.L0_0_0_1_0_1_3_3_col_end_change, names="value")
        self.hs["DnSampRat text"].observe(self.L0_0_0_1_0_1_3_4_downsample_ratio_text_change, names='value')
        self.hs["RoiConfig box"].children = [self.hs["RoiSliStart text"],
        ...
        layout = {"width": "24%"}
        self.hs["UseAltFlat chbx"] = widgets.Checkbox(value=False, description="Alt Flat",
                                                      disabled=True, indent=False)
        self.hs["UseAltFlat chbx"].layout = layout
        layout = {"width": "15%"}
        self.hs["AltFlatFile btn"] = SelectFilesButton(option="askopenfilename",
        ...
        self.hs["UseAltDark chbx"] = widgets.Checkbox(value=False, description="Alt Dark",
                                                      disabled=True, indent=False)
        self.hs["UseAltDark chbx"].layout = layout
        layout = {'left': '9%', "width": "15%"}
        self.hs["AltDarkFile btn"] = SelectFilesButton(option="askopenfilename",
                                                       **{"open_filetypes": (("h5 files", "*.h5"),)})
        ...
        self.hs["UseAltFlat chbx"].observe(self.L0_0_0_1_0_1_1_0_use_alt_flat_checkbox_change, names="value")
        self.hs["AltFlatFile btn"].observe(self.L0_0_0_1_0_1_1_1_alt_flat_file_btn_change, names="value")
        self.hs["UseAltDark chbx"].observe(self.L0_0_0_1_0_1_1_2_use_alt_dark_checkbox_change, names="value")
        self.hs["AltDarkFile btn"].observe(self.L0_0_0_1_0_1_1_3_alt_dark_file_btn_change, names="value")
        self.hs["AltFlatDarkOptn box"].children = [self.hs["UseAltFlat chbx"],
        ...
        layout = {"width": "19%"}
        self.hs["UseFakeFlat chbx"] = widgets.Checkbox(value=False, description="Fake Flat",
                                                       disabled=True, indent=False)
        self.hs["UseFakeFlat chbx"].layout = layout
        layout = {"width": "19%"}
        self.hs["FakeFlatVal text"] = widgets.BoundedFloatText(value=10000.0, description="Flat Val",
                                                               min=100, max=65000, disabled=True)
        self.hs["FakeFlatVal text"].layout = layout
        layout = {"left": "10%", "width": "19%"}
        self.hs["UseFakeDark chbx"] = widgets.Checkbox(value=False, description="Fake Dark",
                                                       disabled=True, indent=False)
        self.hs["UseFakeDark chbx"].layout = layout
        layout = {"left": "19%", "width": "19%"}
        self.hs["FakeDarkVal text"] = widgets.BoundedFloatText(value=100.0, description="Dark Val",
                                                               min=0, max=500, disabled=True)
        self.hs["FakeDarkVal text"].layout = layout

        self.hs["UseFakeFlat chbx"].observe(self.L0_0_0_1_0_1_2_0_use_fake_flat_checkbox_change, names="value")
        self.hs["FakeFlatVal text"].observe(self.L0_0_0_1_0_1_2_1_fake_flat_val_text_change, names="value")
        self.hs["UseFakeDark chbx"].observe(self.L0_0_0_1_0_1_2_2_use_fake_dark_checkbox_change, names="value")
        self.hs["FakeDarkVal text"].observe(self.L0_0_0_1_0_1_2_3_fake_dark_val_text_change, names="value")
        self.hs["FakeFlatDarkOptn box"].children = [self.hs["UseFakeFlat chbx"],
        ...
        layout = {"width": "10%"}
        self.hs["UseBlurFlat chbx"] = widgets.Checkbox(value=False, description="Blur Flat",
                                                    disabled=True, indent=False)
        self.hs["UseBlurFlat chbx"].layout = layout
        layout = {"width": "17%"}
        self.hs["BlurKern text"] = widgets.BoundedIntText(value=20, description='Blur Kernel',
                                                          min=2, max=200, disabled=True)
        self.hs["BlurKern text"].layout = layout
        layout = {"left": "5%", "width": "10%"}
        self.hs["UseRmZinger chbx"] = widgets.Checkbox(value=False, description="Rm Zinger",
                                                    disabled=True, indent=False)
        ...
        self.hs["UseRmZinger chbx"].observe(self.L0_0_0_1_0_1_4_0_rm_zinger_checkbox_change, names="value")
        self.hs["ZingerLevel text"].observe(self.L0_0_0_1_0_1_4_1_zinger_level_text_change, names="value")
        self.hs["UseMask chbx"].observe(self.L0_0_0_1_0_1_4_2_use_mask_checkbox_change, names="value")
        self.hs["MaskRat text"].observe(self.L0_0_0_1_0_1_4_3_mask_ratio_text_change, names="value")
        ...
        self.hs["MiscOptn box"].children = [self.hs["UseBlurFlat chbx"],
        ...
        layout = {"width": "10%"}
        self.hs["IsWedge chbx"] = widgets.Checkbox(value=False, description="Is Wedge",
                                                   disabled=True, indent=False)
        self.hs["IsWedge chbx"].layout = layout
        layout = {"width": "20%"}
        self.hs["MissIdxStart text"] = widgets.BoundedIntText(value=500, min=0, max=5000,
                                                              description="Miss S", disabled=True, )
        self.hs["MissIdxStart text"].layout = layout
        layout = {"width": "20%"}
        self.hs["MissIdxEnd text"] = widgets.BoundedIntText(value=600, min=0, max=5000,
                                                            description="Miss E", disabled=True)
        self.hs["MissIdxEnd text"].layout = layout
        layout = {"width": "20%"}
        self.hs["AutoDet chbx"] = widgets.Checkbox(value=True, description="Auto Det",
                                                   disabled=True, indent=True)
        self.hs["AutoDet chbx"].layout = layout
        layout = {"left": "5%", "width": "20%"}
        self.hs["AutoThres text"] = widgets.BoundedFloatText(value=0.2, min=0, max=1,
                                                             description="Auto Thres", disabled=True)
        self.hs["AutoThres text"].layout = layout

        self.hs["IsWedge chbx"].observe(self.L0_0_1_0_0_1_0_0_is_wedge_checkbox_change, names="value")
        self.hs["MissIdxStart text"].observe(self.L0_0_1_0_0_1_0_2_missing_start_text_change, names="value")
        self.hs["MissIdxEnd text"].observe(self.L0_0_1_0_0_1_0_3_missing_end_text_change, names="value")
        self.hs["AutoDet chbx"].observe(self.L0_0_0_1_0_1_5_4_auto_detect_checkbox_change, names="value")
        self.hs["AutoThres text"].observe(self.L0_0_0_1_0_1_5_5_auto_thres_text_change, names="value")
        self.hs["WedgeOptn box"].children = [self.hs["IsWedge chbx"],
        ...
        layout = {"width": "15%"}
        self.hs["AutoRefFn btn"] = SelectFilesButton(option="askopenfilename",
                                                     **{"open_filetypes": (("h5 files", "*.h5"),)})
        self.hs["AutoRefFn btn"].layout = layout
        self.hs["AutoRefFn btn"].disabled = True

        layout = {"width": "40%"}
        self.hs["AutoRefSli sldr"] = widgets.IntSlider(description="slice #", min=0, max=10, value=0,
                                                       disabled=True, continuous_update=False)
        self.hs["AutoRefSli sldr"].layout = layout

        self.hs["AutoRefColStart text"] = widgets.BoundedIntText(value=0, min=0, max=400,
                                                                 description="W Col_Start", disabled=True)
        layout = {"left": "2.5%", "width": "19%"}
        self.hs["AutoRefColStart text"].layout = layout
        self.hs["AutoRefColEnd text"] = widgets.BoundedIntText(value=10, min=1, max=401,
                                                               description="W Col_End", disabled=True)
        layout = {"left": "2.5%", "width": "19%"}
        self.hs["AutoRefColEnd text"].layout = layout

        self.hs["AutoRefFn btn"].on_click(self.L0_0_1_0_0_1_1_2_auto_ref_fn_btn_change)
        self.hs["AutoRefSli sldr"].observe(self.L0_0_0_1_0_1_6_3_auto_ref_sli_slider_change, names='value')
        self.hs["AutoRefColStart text"].observe(self.L0_0_0_1_0_1_6_4_auto_ref_col_start_text_change, names='value')
        self.hs["AutoRefColEnd text"].observe(self.L0_0_0_1_0_1_6_5_auto_ref_col_end_text_change, names='value')
        self.hs["WedgeRef box"].children = [self.hs["AutoRefFn btn"],
                                            self.hs["AutoRefSli sldr"],
                                            self.hs["AutoRefColStart text"],
                                            self.hs["AutoRefColEnd text"]]
        ## ## ## ## ## ## ## config sub-boxes in data_config_box -- end

        self.hs["DataConfig box"].children = [self.hs["ReconConfig box"],
        ...
        layout = {"border": "3px solid #8855AA", "width": f"{self.form_sz[1] - 121}px",
        ...
        layout = {"width": "24%"}
        self.hs["AlgOptn drpdn"] = widgets.Dropdown(value="gridrec",
                                                    options=["gridrec", "sirt", "tv", "mlem", "astra"],
                                                    description="algs", disabled=True)
        self.hs["AlgOptn drpdn"].layout = layout
        layout = {"width": "23.5%"}
        self.hs["AlgPar00 drpdn"] = widgets.Dropdown(value="", options=[""],
                                                     description="p00", disabled=True)
        self.hs["AlgPar00 drpdn"].layout = layout
        layout = {"width": "23.5%"}
        self.hs["AlgPar01 drpdn"] = widgets.Dropdown(value="", options=[""],
                                                     description="p01", disabled=True)
        self.hs["AlgPar01 drpdn"].layout = layout
        layout = {"width": "23.5%"}
        self.hs["AlgPar02 drpdn"] = widgets.Dropdown(value="", options=[""],
                                                     description="p02", disabled=True)
        self.hs["AlgPar02 drpdn"].layout = layout

        self.hs["AlgOptn drpdn"].observe(self.L0_0_0_1_1_0_1_0_alg_options_dropdown_change, names="value")
        self.hs["AlgPar00 drpdn"].observe(self.L0_0_0_1_1_0_1_1_alg_p00_dropdown_change, names="value")
        self.hs["AlgPar01 drpdn"].observe(self.L0_0_0_1_1_0_1_2_alg_p01_dropdown_change, names="value")
        self.hs["AlgPar02 drpdn"].observe(self.L0_0_0_1_1_0_1_3_alg_p02_dropdown_change, names="value")
        self.hs["AlgOptn box0"].children = [self.hs["AlgOptn drpdn"],
        ...
        layout = {"width": "23.5%"}
        self.hs["AlgPar03 text"] = widgets.FloatText(value=0, description="p03",
                                                     disabled=True)
        self.hs["AlgPar03 text"].layout = layout
        layout = {"width": "23.5%"}
        self.hs["AlgPar04 text"] = widgets.FloatText(value=0, description="p04",
                                                     disabled=True)
        self.hs["AlgPar04 text"].layout = layout
        layout = {"width": "23.5%"}
        self.hs["AlgPar05 text"] = widgets.FloatText(value=0, description="p05",
                                                     disabled=True)
        self.hs["AlgPar05 text"].layout = layout
        layout = {"width": "23.5%"}
        self.hs["AlgPar06 text"] = widgets.FloatText(value=0.0, description="p06",
                                                     disabled=True)
        self.hs["AlgPar06 text"].layout = layout

        self.hs["AlgPar03 text"].observe(self.L0_0_0_1_1_0_2_0_alg_p03_text_change, names="value")
        self.hs["AlgPar04 text"].observe(self.L0_0_0_1_1_0_2_1_alg_p04_text_change, names="value")
        self.hs["AlgPar05 text"].observe(self.L0_0_0_1_1_0_2_2_alg_p05_text_change, names="value")
        self.hs["AlgPar06 text"].observe(self.L0_0_0_1_1_0_2_3_alg_p06_text_change, names="value")
        self.hs["AlgOptn box1"].children = [self.hs["AlgPar03 text"],
                                            self.hs["AlgPar04 text"],
                                            self.hs["AlgPar05 text"],
                                            self.hs["AlgPar06 text"]]

        self.hs["AlgConfig box"].children = [self.hs["AlgOptn box0"],
        ...
        self.hs["DataInfo text"] = widgets.Textarea(value='Data Info', placeholder='Data Info',
                                                    description='Data Info', disabled=True)
        ...
        layout = {"border": "3px solid #FFCC00", "width": f"{self.form_sz[1] - 127}px",
                  "height": f"{0.07 * (self.form_sz[0] - 136)}px"}
        self.hs["ProjPrev box"] = widgets.HBox()
        self.hs["ProjPrev box"].layout = layout
        layout = {"width": "50%"}
        self.hs["RawProj sldr"] = widgets.IntSlider(value=0, description="proj",
                                                    ...
                                                    min=-100, max=100, disabled=True, indent=False,
                                                    continuous_update=False)
        self.hs["RawProj sldr"].layout = layout
        layout = {"width": "20%"}
        self.hs["RawProjInMem chbx"] = widgets.Checkbox(description="read in mem",
        ...
        layout = {"width": "20%"}
        self.hs["RawProjViewerClose btn"] = widgets.Button(description="Close/Confirm",
        ...
        self.hs["RawProj sldr"].observe(self.L0_0_0_1_3_0_3_0_raw_proj_slider_change, names="value")
        self.hs["RawProjInMem chbx"].observe(self.L0_0_0_1_3_0_3_1_raw_proj_all_in_mem_checkbox_click, names='value')
        self.hs["RawProjViewerClose btn"].on_click(self.L0_0_0_1_3_0_3_2_raw_proj_viewer_close_btn_click)
        self.hs["ProjPrev box"].children = [self.hs["RawProj sldr"],
        ...
        layout = {"width": "50%"}
        self.hs["CenOffsetRange sldr"] = widgets.IntSlider(value=0, description="offset",
                                                           description_tooltip='offset the image at 180 deg to overlap with the image at 0 deg. rotation center can be determined from the offset.',
                                                           min=-100, max=100, disabled=True,
                                                           indent=False, continuous_update=False)
        self.hs["CenOffsetRange sldr"].layout = layout
        layout = {"width": "20%"}
        self.hs["CenOffsetCfm btn"] = widgets.Button(description="Confirm",
        ...
        layout = {"width": "20%"}
        self.hs["CenViewerClose btn"] = widgets.Button(description="Close",
        ...
        self.hs["CenOffsetRange sldr"].observe(self.L0_0_0_1_3_0_0_0_cen_offset_range_slider_change, names="value")
        self.hs["CenOffsetCfm btn"].on_click(self.L0_0_0_1_3_0_0_1_cen_offset_confirm_btn_click)
        self.hs["CenViewerClose btn"].on_click(self.L0_0_0_1_3_0_0_2_cen_viewer_close_btn_click)
        self.hs["CenPrev box"].children = [self.hs["CenOffsetRange sldr"],
        ...
        layout = {"width": "50%"}
        self.hs["TrialCenPrev sldr"] = widgets.IntSlider(value=0, description="trial cen",
                                                         description_tooltip='offset the image at 180 deg to overlap with the image at 0 deg. rotation center can be determined from the offset.',
                                                         min=-100, max=100, disabled=True,
                                                         indent=False, continuous_update=False)
        self.hs["TrialCenPrev sldr"].layout = layout
        layout = {"width": "20%"}
        self.hs["TrialCenCfm btn"] = widgets.Button(description="Confirm",
                                                    description_tooltip='Optional confrimation of the rough center',
                                                    disabled=True)
        self.hs["TrialCenCfm btn"].layout = layout
        self.hs["TrialCenCfm btn"].style.button_color = 'darkviolet'
        layout = {"width": "20%"}
        self.hs["TrialCenViewerClose btn"] = widgets.Button(description="Close",
                                                            description_tooltip='Optional close the viewer window',
                                                            disabled=True)
        self.hs["TrialCenViewerClose btn"].layout = layout
        self.hs["TrialCenViewerClose btn"].style.button_color = 'darkviolet'

        self.hs["TrialCenPrev sldr"].observe(self.L0_0_0_1_3_0_1_0_trial_cen_slider_change, names="value")
        self.hs["TrialCenCfm btn"].on_click(self.L0_0_0_1_3_0_1_1_trial_cen_confirm_btn_click)
        self.hs["TrialCenViewerClose btn"].on_click(self.L0_0_0_1_3_0_1_2_trial_cen_viewer_close_btn_click)
        self.hs["TrialCenPrev box"].children = [self.hs["TrialCenPrev sldr"],
        ...
        layout = {"border": "3px solid #FFCC00", "width": f"{self.form_sz[1] - 127}px",
                  "height": f"{0.07 * (self.form_sz[0] - 136)}px"}
        self.hs["VolSliPrev box"] = widgets.HBox()
        self.hs["VolSliPrev box"].layout = layout
        layout = {"width": "50%"}
        self.hs["VolSliPrev sldr"] = widgets.IntSlider(value=0, description="sli",
                                                       description_tooltip='offset the image at 180 deg to overlap with the image at 0 deg. rotation center can be determined from the offset.',
                                                       min=-100, max=100, disabled=True, indent=False,
                                                       continuous_update=False)
        self.hs["VolSliPrev sldr"].layout = layout
        layout = {"width": "20%"}
        self.hs["VolSliViewerClose btn"] = widgets.Button(description="Close", disabled=True,
                                                          description_tooltip='Optional confrimation of the rough center')
        self.hs["VolSliViewerClose btn"].layout = layout
        self.hs["VolSliViewerClose btn"].style.button_color = 'darkviolet'

        self.hs["VolSliPrev sldr"].observe(self.L0_0_0_1_3_0_2_0_vol_sli_slider_change, names="value")
        self.hs["VolSliViewerClose btn"].on_click(self.L0_0_0_1_3_0_2_1_vol_sli_viewer_close_btn_click)
        self.hs["VolSliPrev box"].children = [self.hs["VolSliPrev sldr"],
                                              self.hs["VolSliViewerClose btn"]]
        ## ## ## ## ## ## ## define functional box widgets in data_prevew_box2 -- end

        self.hs["DataPrev box"].children = [self.hs["ProjPrev box"],
                                            self.hs["CenPrev box"],
                                            self.hs["TrialCenPrev box"],
                                            self.hs["VolSliPrev box"]]
        ## ## ## ## ## ## define data_preview_box-- end

        self.hs["DataPrev tab"].children = [self.hs["DataPrev box"]]
        ## ## ## ## ## define data_preview tab -- start

        self.hs['Config&Input form'].children = [self.hs["SelFile&Path box"],
                                                 self.hs["Data tab"]]
        ## ## ## config config_input_form -- end

        ## ## ## ## config filter&recon tab -- start
        layout = {"border": "3px solid #8855AA", "width": f"{self.form_sz[1] - 122}px",
                  "height": f"{0.88 * (self.form_sz[0] - 136)}px"}
        ...
        grid_recon_chunk = GridspecLayout(2, 100, layout={"border": "3px solid #FFCC00",
                                                          ...
                                                          "width": f"{self.hs['Filter&Recon box'].layout.width}",
                                                          "grid_row_gap": "4px",
                                                          "grid_column_gap": "8px",
        ...
        grid_recon_chunk[0, 30:70] = widgets.HTML(
            "<span style='color:red; font-size: 150%; font-weight: bold; background-color:rgb(135,206,250);'>" + "Chunk&Margin" + "</span>")
        self.hs["ChunkMargTitle label"] = grid_recon_chunk[0, 30:70]
        layout = {"left": f"{0.41 * (int(self.hs['Filter&Recon tab'].layout.width.strip('px')) - 6)}px",
        ...
        grid_recon_chunk[1, :10] = widgets.BoundedIntText(description='Chunk Sz', disabled=True,
                                                          min=1, max=2000,
                                                          layout={'width': f'{int(0.20 * (self.form_sz[1] - 136))}px',
                                                                  'height': '90%'},
                                                          value=200,
        ...
        self.hs["ReconChunkSz text"].observe(self.L0_0_1_0_0_2_1_recon_chunk_sz_text_change, names="value")
        self.hs["ReconMargSz text"].observe(self.L0_0_1_0_0_2_2_recon_margin_sz_text_change, names="value")
        self.hs["ReconChunk box"].children = [self.hs["ChunkMargTitle label"],
        ...
        layout = {"justify-content": "center", "align-items": "center", "align-contents": "center",
                  "border": "3px solid #FFCC00", "height": f"{0.07 * (self.form_sz[0] - 136)}px"}
        ...
        self.hs["FilterConfigTitle label"] = widgets.HTML(
            "<span style='color:red; font-size: 150%; font-weight: bold; background-color:rgb(135,206,250);'>" + "Filter Config" + "</span>")
        ...
        FilterConfigGrid = GridspecLayout(2, 200,
        ...
        FilterConfigGrid[0, :100] = GridspecLayout(10, 20,
                                                   ...
                                                   layout={"border": "3px solid #FFCC00",
                                                           ...
                                                           "justify_items": "flex-start",
                                                           "grid_column_gap": "8px"})
        self.hs["FilterConfigLeft box"] = FilterConfigGrid[0, :100]
        FilterConfigGrid[0, :100][0, :16] = widgets.Dropdown(value="phase retrieval",
                                                             options=FILTERLIST,
                                                             description="Filter List",
                                                             indent=False,
                                                             disabled=True)
        self.hs["FilterConfigLeftFltList drpdn"] = FilterConfigGrid[0, :100][0, :16]
        FilterConfigGrid[0, :100][0, 16:19] = widgets.Button(description="==>",
                                                             disabled=True,
                                                             layout={
                                                                 "width": f"{int(1.5 * (self.form_sz[1] - 98) / 20)}px"})
        self.hs["FilterConfigLeftAddTo btn"] = FilterConfigGrid[0, :100][0, 16:19]
        FilterConfigGrid[0, :100][0, 16:19].style.button_color = "#0000FF"
        for ii in range(3):
            for jj in range(2):
                FilterConfigGrid[0, :100][1 + ii, jj * 8:(jj + 1) * 8] = widgets.Dropdown(value="",
                                                                                          options=[""],
                                                                                          description="p" + str(
                                                                                              ii * 2 + jj).zfill(2),
                                                                                          disabled=True,
        ...
        for ii in range(3):
            for jj in range(2):
                FilterConfigGrid[0, :100][4 + ii, jj * 8:(jj + 1) * 8] = widgets.FloatText(value=0, disabled=True,
                                                                                           description="p" + str(
                                                                                               (ii + 3) * 2 + jj).zfill(
                                                                                               2),
                                                                                           layout={
        ...
        self.hs["FilterConfigLeftPar00 text"] = FilterConfigGrid[0, :100][1, 0:8]
        self.hs["FilterConfigLeftPar01 text"] = FilterConfigGrid[0, :100][1, 8:16]
        self.hs["FilterConfigLeftPar02 text"] = FilterConfigGrid[0, :100][2, 0:8]
        self.hs["FilterConfigLeftPar03 text"] = FilterConfigGrid[0, :100][2, 8:16]
        self.hs["FilterConfigLeftPar04 text"] = FilterConfigGrid[0, :100][3, 0:8]
        self.hs["FilterConfigLeftPar05 text"] = FilterConfigGrid[0, :100][3, 8:16]
        self.hs["FilterConfigLeftPar06 text"] = FilterConfigGrid[0, :100][4, 0:8]
        self.hs["FilterConfigLeftPar07 text"] = FilterConfigGrid[0, :100][4, 8:16]
        self.hs["FilterConfigLeftPar08 text"] = FilterConfigGrid[0, :100][5, 0:8]
        self.hs["FilterConfigLeftPar09 text"] = FilterConfigGrid[0, :100][5, 8:16]
        self.hs["FilterConfigLeftPar10 text"] = FilterConfigGrid[0, :100][6, 0:8]
        self.hs["FilterConfigLeftPar11 text"] = FilterConfigGrid[0, :100][6, 8:16]

        FilterConfigGrid[0, :100][7:, :] = widgets.HTML(
            value="<style>p{word-wrap: break-word}</style> <p>" + "Hover mouse over params for the description of the param for each filter." + " </p>")
        self.hs["FilterConfigLeftFltList drpdn"].observe(
            self.L0_0_1_0_0_1_0_0_filter_config_box_left_box_filter_dropdown_change, names="value")
        self.hs["FilterConfigLeftAddTo btn"].on_click(self.L0_0_1_0_0_1_0_1_filter_config_box_left_box_move_btn_change)
        self.hs["FilterConfigLeftPar00 text"].observe(self.L0_0_1_0_0_1_0_2_filter_config_box_left_box_p00_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar01 text"].observe(self.L0_0_1_0_0_1_0_3_filter_config_box_left_box_p01_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar02 text"].observe(self.L0_0_1_0_0_1_0_4_filter_config_box_left_box_p02_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar03 text"].observe(self.L0_0_1_0_0_1_0_5_filter_config_box_left_box_p03_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar04 text"].observe(self.L0_0_1_0_0_1_0_6_filter_config_box_left_box_p04_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar05 text"].observe(self.L0_0_1_0_0_1_0_7_filter_config_box_left_box_p05_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar06 text"].observe(self.L0_0_1_0_0_1_0_8_filter_config_box_left_box_p06_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar07 text"].observe(self.L0_0_1_0_0_1_0_9_filter_config_box_left_box_p07_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar08 text"].observe(self.L0_0_1_0_0_1_0_10_filter_config_box_left_box_p08_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar09 text"].observe(self.L0_0_1_0_0_1_0_11_filter_config_box_left_box_p09_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar10 text"].observe(self.L0_0_1_0_0_1_0_12_filter_config_box_left_box_p10_change,
                                                      names="value")
        self.hs["FilterConfigLeftPar11 text"].observe(self.L0_0_1_0_0_1_0_13_filter_config_box_left_box_p11_change,
                                                      names="value")
        self.hs["FilterConfigLeft box"].children = [self.hs["FilterConfigLeftFltList drpdn"],
        ...
        FilterConfigGrid[0, 100:] = GridspecLayout(10, 10,
                                                   grid_gap="8px",
                                                   layout={"border": "3px solid #FFCC00",
                                                           "height": f"{0.50 * (self.form_sz[0] - 136)}px"})
        self.hs["FilterConfigRight box"] = FilterConfigGrid[0, 100:]
        FilterConfigGrid[0, 100:][:, :9] = widgets.SelectMultiple(value=["None"],
                                                                  options=["None"],
                                                                  description="Filter Seq",
                                                                  disabled=True,
                                                                  layout={
                                                                      "height": f"{0.48 * (self.form_sz[0] - 136)}px"})
        self.hs["FilterConfigRightFlt mulsel"] = FilterConfigGrid[0, 100:][:, :9]
        FilterConfigGrid[0, 100:][1, 9] = widgets.Button(description="Move Up",
                                                         disabled=True,
                                                         layout={"width": f"{int(2 * (self.form_sz[1] - 98) / 20)}px"})
        FilterConfigGrid[0, 100:][1, 9].style.button_color = "#0000FF"
        self.hs["FilterConfigRightMvUp btn"] = FilterConfigGrid[0, 100:][1, 9]
        FilterConfigGrid[0, 100:][2, 9] = widgets.Button(description="Move Dn",
                                                         disabled=True,
                                                         layout={"width": f"{int(2 * (self.form_sz[1] - 98) / 20)}px"})
        FilterConfigGrid[0, 100:][2, 9].style.button_color = "#0000FF"
        self.hs["FilterConfigRightMvDn btn"] = FilterConfigGrid[0, 100:][2, 9]
        FilterConfigGrid[0, 100:][3, 9] = widgets.Button(description="Remove",
                                                         disabled=True,
                                                         layout={"width": f"{int(2 * (self.form_sz[1] - 98) / 20)}px"})
        FilterConfigGrid[0, 100:][3, 9].style.button_color = "#0000FF"
        self.hs["FilterConfigRightRm btn"] = FilterConfigGrid[0, 100:][3, 9]
        FilterConfigGrid[0, 100:][4, 9] = widgets.Button(description="Finish",
                                                         disabled=True,
                                                         layout={"width": f"{int(2 * (self.form_sz[1] - 98) / 20)}px"})
        FilterConfigGrid[0, 100:][4, 9].style.button_color = "#0000FF"
        self.hs["FilterConfigRightFnsh btn"] = FilterConfigGrid[0, 100:][4, 9]

        self.hs["FilterConfigRightFlt mulsel"].observe(
            self.L0_0_1_0_0_1_1_0_filter_config_box_right_box_selectmultiple_change, names="value")
        self.hs["FilterConfigRightMvUp btn"].on_click(
            self.L0_0_1_0_0_1_1_1_filter_config_box_right_box_move_up_btn_change)
        self.hs["FilterConfigRightMvDn btn"].on_click(
            self.L0_0_1_0_0_1_1_2_filter_config_box_right_box_move_dn_btn_change)
        self.hs["FilterConfigRightRm btn"].on_click(self.L0_0_1_0_0_1_1_3_filter_config_box_right_box_remove_btn_change)
        self.hs["FilterConfigRightFnsh btn"].on_click(
            self.L0_0_1_0_0_1_1_4_filter_config_box_right_box_finish_btn_change)
        self.hs["FilterConfigRight box"].children = [self.hs["FilterConfigRightFlt mulsel"],
        ...
        FilterConfigGrid[1, :140] = widgets.Text(
            ...
            disabled=True)
        self.hs["FilterConfigCfm text"] = FilterConfigGrid[1, :140]
        FilterConfigGrid[1, 141:171] = widgets.Button(description="Confirm",
                                                      disabled=True,
                                                      layout={"width": f"{int(0.15 * (self.form_sz[1] - 136))}px",
                                                              "height": "90%"})
        FilterConfigGrid[1, 141:171].style.button_color = "darkviolet"
        self.hs["FilterConfigCfm btn"] = FilterConfigGrid[1, 141:171]
        self.hs["FilterConfigCfm btn"].on_click(self.L0_0_1_0_0_1_3_filter_config_box_confirm_btn)
        ## ## ## ## ## ## ## config confirm box in GridspecLayout -- end

        self.hs["FilterConfig box"] = FilterConfigGrid
        self.hs["FilterConfig box"].children = [self.hs["FilterConfigLeft box"],
        ...
        self.hs["Filter&Recon box"].children = [self.hs["ReconChunk box"],
        ...
        self.hs["ReconPrgr bar"] = widgets.IntProgress(value=0, min=0, max=10, step=1,
        ...
        self.hs["Recon btn"].on_click(self.L0_0_1_0_1_1_1_recon_btn_click)
        self.hs["ReconDo box"].children = [self.hs["ReconPrgr bar"],
                                           self.hs["Recon btn"]]
        ## ## ## ## ## ## ## config widgets in recon_box -- end

        self.hs["Recon box"].children = [self.hs["Filter&Recon box"],
                                         self.hs["ReconDo box"]]
        ## ## ## ## ## config recon box -- end

        self.hs["Filter&Recon form"].children = [self.hs["Filter&Recon box"],
                                                 self.hs["ReconDo box"]]
        ## ## ## ## config filter&recon tab -- end

        ## ## ## ## config filter&recon tab -- start
        layout = {"border": "3px solid #8855AA", "width": f"{self.form_sz[1] - 136}px",
                  "height": f"{0.84 * (self.form_sz[0] - 122)}px"}
        self.hs["ReconConfigSumm form"] = widgets.VBox()
        self.hs["ReconConfigSumm form"].layout = layout

        ## ## ## ## ## config filter&recon text -- start
        layout = {"width": "95%", "height": "90%"}
        self.hs["ReconConfigSumm text"] = widgets.Textarea(value='Recon Config Info',
        ...
        self.hs["Filter&Recon tab"].children = [self.hs["Filter&Recon form"],
    ...
    def bundle_param_handles(self):
        self.flt_phs = [self.hs["FilterConfigLeftPar00 text"],
                        self.hs["FilterConfigLeftPar01 text"],
                        self.hs["FilterConfigLeftPar02 text"],
                        self.hs["FilterConfigLeftPar03 text"],
                        self.hs["FilterConfigLeftPar04 text"],
                        ...
                        self.hs["FilterConfigLeftPar10 text"],
                        self.hs["FilterConfigLeftPar11 text"]]
        self.alg_phs = [self.hs["AlgPar00 drpdn"],
                        self.hs["AlgPar01 drpdn"],
                        self.hs["AlgPar02 drpdn"],
    ...
    def boxes_logic(self):
        ...
        if not self.tomo_filepath_configured:
            ...
            self.lock_message_text_boxes()
              ...
              (self.recon_finish == 0) & (self.tomo_recon_type == "Trial Cent")):
            boxes = ["DataConfig tab",
                     "AlgOptn drpdn",
                     "DataInfo tab",
                     "CenPrev box",
        ...
        if self.data_info is None:
            ...
            if self.data_info:
                if self.hs["CenWinLeft text"].value >= (
                    ...
                    self.hs["CenWinLeft text"].value = int(
                        self.data_info["img_dim"][2] / 2) - 40
                else:
                    self.hs["CenWinLeft text"].max = self.data_info["img_dim"][2] - self.hs[
                        "CenWinWz text"].value - 1
        else:
            data_info = get_raw_img_info(self.tomo_raw_data_file_template.format(self.tomo_scan_id),
                                         self.global_h.io_tomo_cfg, scan_type='tomo')
            if data_info:
                info = ""
    ...
    def set_rec_dict_from_rec_params(self):
        ...
        self.tomo_recon_param_dict["recon_config"]["use_flat_blur"] = self.tomo_use_blur_flat
        ...
        self.tomo_recon_param_dict["data_params"]["blur_kernel"] = self.tomo_flat_blur_kernel
    ...
    def L0_0_0_0_1_0_select_raw_h5_top_dir_btn_click(self, a):
        self.reset_config()
        if len(a.files[0]) != 0:
            self.tomo_raw_data_top_dir = a.files[0]
            self.tomo_recon_top_dir = a.files[0]
            self.tomo_raw_data_file_template = os.path.join(self.tomo_raw_data_top_dir,
            ...
            update_json_content(self.global_h.GUI_cfg_file, {'cwd': os.path.abspath(a.files[0])})
        else:
            self.tomo_raw_data_top_dir = None
            self.tomo_raw_data_top_dir_set = False
            self.tomo_recon_top_dir = None
        ...
        self.tomo_filepath_configured = False
        self.recon_finish = -1
        self.hs["SelFile&PathCfm text"].value = "After setting directories, confirm to proceed ..."
        self.boxes_logic()

    def L0_0_0_0_2_0_select_save_recon_dir_btn_click(self, a):
        self.reset_config()
        if not self.tomo_raw_data_top_dir_set:
            self.hs["SelFile&PathCfm text"].value = "Please specify raw h5 top directory first ..."
            self.hs["SelSavReconDir text"].value = "Choose top directory where recon subdirectories are saved..."
            self.tomo_recon_path_set = False
        else:
            if len(a.files[0]) != 0:
                self.tomo_recon_top_dir = a.files[0]
                ...
                self.tomo_recon_top_dir = None
                self.tomo_recon_path_set = False
                self.hs["SelSavReconDir text"].value = "Select top directory where recon subdirectories are saved..."
            self.hs["SelFile&PathCfm text"].value = "After setting directories, confirm to proceed ..."
        self.tomo_filepath_configured = False
        self.recon_finish = -1
        self.boxes_logic()

    def L0_0_0_0_4_0_select_save_debug_dir_btn_click(self, a):
        self.reset_config()
        if not self.tomo_raw_data_top_dir_set:
            self.hs["SelFile&PathCfm text"].value = "Please specify raw h5 top directory first ..."
            self.hs["SelSavDebugDir text"].value = "Select top directory where debug dir will be created..."
            self.tomo_debug_path_set = False
        else:
            if len(a.files[0]) != 0:
    ...
    def L0_0_0_0_5_2_recon_option_dropdown_change(self, a):
        ...
        if self.tomo_recon_type == "Trial Cent":
            ...
            self.hs["SelSavReconDir btn"].disabled = False
            self.hs["SelSavReconDir btn"].style.button_color = "orange"
            self.hs["SelSavReconDir text"].value = "Select top directory where data_center directory will be created..."
    ...
    def L0_0_0_0_5_0_confirm_file_path_btn_click(self, a):
        if self.tomo_recon_type == "Trial Cent":
            if (self.tomo_raw_data_top_dir_set & self.tomo_recon_path_set):
                self.tomo_available_raw_idx = check_file_availability(self.tomo_raw_data_top_dir, scan_id=None,
                                                                      signature=self.global_h.io_tomo_cfg[
                                                                      'tomo_raw_fn_template'], return_idx=True)
                if len(self.tomo_available_raw_idx) == 0:
                    ...
                    if self.tomo_use_debug:
                        if self.tomo_debug_path_set:
                            self.tomo_data_center_path = os.path.join(self.tomo_recon_top_dir, "data_center")
                            self.tomo_filepath_configured = True
                            self.set_alg_param_widgets()
                            self.set_flt_param_widgets()
                        else:
                            self.hs[
                                "SelFile&PathCfm text"].value = "You need to select the top directory to create debug dir..."
                            self.tomo_filepath_configured = False
                    else:
                        self.tomo_data_center_path = os.path.join(self.tomo_recon_top_dir, "data_center")
                        self.tomo_filepath_configured = True
                ...
                self.hs[
                    "SelFile&PathCfm text"].value = "You need to select the top raw dir and top dir where debug dir can be created..."
                self.tomo_filepath_configured = False
        elif self.tomo_recon_type == "Vol Recon":
            if (self.tomo_raw_data_top_dir_set & self.tomo_recon_path_set):
                self.tomo_available_raw_idx = check_file_availability(self.tomo_raw_data_top_dir, scan_id=None,
                ...
                self.hs[
                    "SelFile&PathCfm text"].value = "You need to select the top raw dir and top dir where recon dir can be created..."
                self.tomo_filepath_configured = False
        self.recon_finish = -1
        self.boxes_logic()
    ...
    def L0_0_0_1_0_1_0_0_scan_id_dropdown_change(self, a):
        ...
        if self.data_info:
            if self.tomo_recon_type == 'Trial Cent':
                ...
                self.hs["RoiSliEnd text"].value = int(self.data_info["img_dim"][1] / 2) + 10
                self.hs["RoiSliStart text"].value = 0
                self.hs["RoiSliStart text"].max = self.data_info["img_dim"][1] - 2
                self.hs["RoiSliStart text"].value = int(self.data_info["img_dim"][1] / 2) - 10
                ...
                self.hs["RoiSliEnd text"].max = self.data_info["img_dim"][1] - 1
                if self.hs["RoiSliStart text"].value > (self.data_info["img_dim"][1] - 2):
                    self.hs["RoiSliStart text"].value = (self.data_info["img_dim"][1] - 2)
                self.hs["RoiSliStart text"].max = self.data_info["img_dim"][1] - 2
                if self.hs["RoiColStart text"].value > (self.data_info["img_dim"][2] - 2):
    ...
    def L0_0_0_1_0_1_0_2_cen_win_left_text_change(self, a):
        if a["owner"].value > (self.data_info["img_dim"][2] - self.hs["CenWinWz text"].value - 1):
            a["owner"].value = (self.data_info["img_dim"][2] - self.hs["CenWinWz text"].value - 1)
    ...
    def L0_0_0_1_0_1_0_3_cen_win_wz_text_change(self, a):
        if a["owner"].value > (self.data_info["img_dim"][2] - self.hs["CenWinLeft text"].value - 1):
            a["owner"].value = (self.data_info["img_dim"][2] - self.hs["CenWinLeft text"].value - 1)
    ...
    def L0_0_0_1_0_1_0_4_read_cen_list_btn_click(self, a):
    ...
    def L0_0_0_1_0_1_1_1_alt_flat_file_btn_change(self, a):
        if len(a.files[0]) != 0:
    ...
    def L0_0_0_1_0_1_1_3_alt_dark_file_btn_change(self, a):
        if len(a.files[0]) != 0:
    ...
    def L0_0_1_0_0_1_1_2_auto_ref_fn_btn_change(self, a):
        if len(a.files[0]) != 0:
            self.tomo_wedge_ang_auto_det_ref_fn = a.files[0]
            self.wedge_eva_data, flat, dark, _ = read_data(self.reader, self.tomo_wedge_ang_auto_det_ref_fn,
    ...
    def L0_0_0_1_0_1_6_3_auto_ref_sli_slider_change(self, a):
        ...
        plt.plot(np.arange(self.wedge_eva_data.shape[0]),
    ...
    def L0_0_0_1_3_0_3_0_raw_proj_slider_change(self, a):
        ...
        if self.load_raw_in_mem:
            if not self.raw_is_in_mem:
                self.raw = ((self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='data',
                                         sli=[None, None, None], cfg=self.global_h.io_tomo_cfg) -
                             self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='dark',
                                         sli=[None, None, None], cfg=self.global_h.io_tomo_cfg).mean(axis=0)) /
                            (self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='flat',
                                         sli=[None, None, None], cfg=self.global_h.io_tomo_cfg).mean(axis=0) -
                             self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='dark',
                                         sli=[None, None, None], cfg=self.global_h.io_tomo_cfg).mean(axis=0))).astype(
                    np.float32)
            ...
            if (not data_state) | (not viewer_state):
                ...
                self.global_h.tomo_fiji_windows['tomo_raw_img_viewer']['ip'].setImage(
                    self.global_h.ij.convert().convert(
                        self.global_h.ij.dataset().create(self.global_h.ij.py.to_java(self.raw)),
            ...
            self.global_h.tomo_fiji_windows['tomo_raw_img_viewer']['ip'].setRoi(self.tomo_col_s, self.tomo_sli_s,
                                                                                self.tomo_col_e - self.tomo_col_s,
                                                                                self.tomo_sli_e - self.tomo_sli_s)
        else:
            self.raw_proj[:] = ((self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='data',
                                             sli=[[idx, idx + 1], None, None], cfg=self.global_h.io_tomo_cfg) -
                                 self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='dark',
                                             sli=[None, None, None], cfg=self.global_h.io_tomo_cfg).mean(axis=0)) /
                                (self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='flat',
                                             sli=[None, None, None], cfg=self.global_h.io_tomo_cfg).mean(axis=0) -
                                 self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='dark',
                                             sli=[None, None, None], cfg=self.global_h.io_tomo_cfg).mean(
            ...
            self.global_h.tomo_fiji_windows['tomo_raw_img_viewer']['ip'].setImage(self.global_h.ij.convert().convert(
                self.global_h.ij.dataset().create(self.global_h.ij.py.to_java(self.raw_proj)),
                self.global_h.ImagePlusClass))
            self.global_h.ij.py.run_macro("""run("Enhance Contrast", "saturated=0.35")""")
            self.global_h.tomo_fiji_windows['tomo_raw_img_viewer']['ip'].setRoi(self.tomo_col_s, self.tomo_sli_s,
                                                                                self.tomo_col_e - self.tomo_col_s,
    ...
    def L0_0_0_1_3_0_3_2_raw_proj_viewer_close_btn_click(self, a):
        data_state, viewer_state = fiji_viewer_state(self.global_h, self, viewer_name='tomo_raw_img_viewer')
    ...
    def L0_0_0_1_3_0_0_0_cen_offset_range_slider_change(self, a):
        self.raw_proj_0[:] = ((self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='data',
                                           sli=[[0, 1], None, None], cfg=self.global_h.io_tomo_cfg) -
                               self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='dark',
                                           sli=[None, None, None], cfg=self.global_h.io_tomo_cfg).mean(axis=0)) /
                              (self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='flat',
                                           sli=[None, None, None], cfg=self.global_h.io_tomo_cfg).mean(axis=0) -
        ...
        self.raw_proj_180[:] = ((self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='data',
                                             sli=[[-2, -1], None, None], cfg=self.global_h.io_tomo_cfg) -
                                 self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='dark',
                                             sli=[None, None, None], cfg=self.global_h.io_tomo_cfg).mean(axis=0)) /
                                (self.reader(self.tomo_raw_data_file_template.format(self.tomo_scan_id), dtype='flat',
                                             sli=[None, None, None], cfg=self.global_h.io_tomo_cfg).mean(axis=0) -
        ...
        self.global_h.tomo_fiji_windows['tomo_0&180_viewer']['ip'].setImage(self.global_h.ij.convert().convert(
            self.global_h.ij.dataset().create(self.global_h.ij.py.to_java(
                np.roll(self.raw_proj_180[:, ::-1], a['owner'].value, axis=1) - self.raw_proj_0)),
            self.global_h.ImagePlusClass))
        self.global_h.ij.py.run_macro("""run("Enhance Contrast", "saturated=0.35")""")

    def L0_0_0_1_3_0_0_1_cen_offset_confirm_btn_click(self, a):
        self.manual_cen = self.hs["CenOffsetRange sldr"].value / 2.
        self.hs["RotCen text"].value = self.manual_cen + self.data_info["img_dim"][2] / 2
    ...
    def L0_0_0_1_3_0_0_2_cen_viewer_close_btn_click(self, a):
        fiji_viewer_off(self.global_h, self, viewer_name='tomo_0&180_viewer')
    ...
    def L0_0_0_1_3_0_1_1_trial_cen_confirm_btn_click(self, a):
        self.trial_cen = self.hs["CenWinLeft text"].value + \
        ...
        try:
            ...
            with open(self.tomo_trial_cen_dict_fn, 'w') as f:
                ...
                tem[str(self.tomo_recon_param_dict["data_params"]["scan_id"])]['file_params']['io_confg'][
    ...
    def L0_0_0_1_3_0_1_2_trial_cen_viewer_close_btn_click(self, a):
        fiji_viewer_off(self.global_h, self, viewer_name='tomo_cen_review_viewer')

    def L0_0_0_1_3_0_2_0_vol_sli_slider_change(self, a):
        pass

    def L0_0_0_1_3_0_2_1_vol_sli_viewer_close_btn_click(self, a):
        pass
    ...
    def L0_0_1_0_0_1_0_1_filter_config_box_left_box_move_btn_change(self, a):
        ...
        if ((len(self.hs["FilterConfigRightFlt mulsel"].options) == 1) &
            ...
            self.hs["FilterConfigRightFlt mulsel"].options = [self.tomo_left_box_selected_flt, ]
            self.hs["FilterConfigRightFlt mulsel"].value = [self.tomo_left_box_selected_flt, ]
            self.tomo_right_filter_dict[0] = {'filter_name': self.tomo_left_box_selected_flt,
            ...
            idx = len(a) - 1
            self.tomo_right_filter_dict[idx] = {'filter_name': self.tomo_left_box_selected_flt,
                                                'params': self.flt_param_dict}
        self.tomo_data_configured = False
        self.recon_finish = -1
        self.boxes_logic()
    ...
    def L0_0_1_0_0_1_1_1_filter_config_box_right_box_move_up_btn_change(self, a):
        if (len(self.hs["FilterConfigRightFlt mulsel"].options) == 1):
            pass
        else:
    ...
    def L0_0_1_0_0_1_1_2_filter_config_box_right_box_move_dn_btn_change(self, a):
        if (len(self.hs["FilterConfigRightFlt mulsel"].options) == 1):
            pass
        else:
            a = np.array(self.hs["FilterConfigRightFlt mulsel"].options)
    ...
    def L0_0_1_0_0_1_1_3_filter_config_box_right_box_remove_btn_change(self, a):
        a = list(self.hs["FilterConfigRightFlt mulsel"].options)
    ...
    def L0_0_1_0_0_1_1_4_filter_config_box_right_box_finish_btn_change(self, a):
        pass

    def L0_0_1_0_0_1_3_filter_config_box_confirm_btn(self, a):
        """
    ...
    def L0_0_1_0_1_1_1_recon_btn_click(self, a):
        if self.tomo_recon_type == "Trial Cent":
            fiji_viewer_off(self.global_h, self, viewer_name='tomo_cen_review_viewer')
            code = {}
            ln = 0
            ...
            ln += 1
            code[ln] = f"    params['file_params']['reader'] = data_reader(tomo_h5_reader)";
            ln += 1
            code[ln] = f"    params['file_params']['info_reader'] = data_info(tomo_h5_info)";
            ln += 1
            code[ln] = f"else:";
            ln += 1
