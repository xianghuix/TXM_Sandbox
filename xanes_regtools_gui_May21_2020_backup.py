...
class xanes_regtools_gui():
    ...
    def determine_fitting_energy_range(self, dtype='3D_XANES'):
        if dtype == '3D_XANES':
            ...
            if self.xanes_element == 'Zn':
                ...
                self.xanes2D_analysis_wl_fit_eng_s = 8.342
                self.xanes2D_analysis_wl_fit_eng_e = 8.355
    ...
    def L0_2_2_1_2_0_reg_pair_slider_change(self, a):
        ...
        self.xanes3D_fiji_windows['xanes3D_review_viewer']['ip'].setImage(ij.convert().convert(ij.dataset().create(ij.py.to_java(self.trial_reg-self.trial_reg_fixed)),
                                                                                                 ImagePlusClass))
        self.xanes3D_fiji_windows['xanes3D_review_viewer']['ip'].show()
        ij.py.run_macro("""run("Enhance Contrast", "saturated=0.35")""")
    ...
    def L0_2_2_1_3_0_zshift_slider_change(self, a):
        self.fiji_viewer_on(viewer_name='xanes3D_review_viewer')
        self.xanes3D_fiji_windows['xanes3D_review_viewer']['ip'].setSlice(a['owner'].value)
    ...
    def L0_2_2_1_3_2_record_button_click(self, a):
        ...
        self.xanes3D_review_shift_dict[str(self.xanes3D_alignment_pair_id)] = np.array([self.hs['L[0][2][2][1][3][1]_best_match_text'].value-1,
                                                                                        shift[self.hs['L[0][2][2][1][3][1]_best_match_text'].value-1][0],
                                                                                        shift[self.hs['L[0][2][2][1][3][1]_best_match_text'].value-1][1]])
    ...
    def L0_2_2_1_5_2_review_manual_z_shift_text(self, a):
        self.xanes3D_manual_xshift = self.hs['L[0][2][2][1][5][0]_review_manual_x_shift_text'].value
        self.xanes3D_manual_yshift = self.hs['L[0][2][2][1][5][1]_review_manual_y_shift_text'].value
        self.xanes3D_manual_zshift = self.hs['L[0][2][2][1][5][2]_review_manual_z_shift_text'].valu
        xanes3D_review_aligned_img = (np.real(np.fft.ifftn(fourier_shift(np.fft.fftn(self.trial_reg[self.xanes3D_manual_zshift-1]),
                                                                                [self.xanes3D_manual_yshift, self.xanes3D_manual_xshift]))))
    ...
    def L0_2_2_1_5_3_review_manual_shift_record_button(self, a):
        ...
        shift = f['/trial_registration/trial_reg_results/{0}/shift{0}'.format(str(self.xanes3D_alignment_pair_id).zfill(3))][:]
        f.close()
        self.xanes3D_review_shift_dict["{}".format(self.xanes3D_alignment_pair_id)] = np.array([self.xanes3D_manual_zshift-1,
                                                                                                self.xanes3D_manual_yshift+shift[self.hs['L[0][2][2][1][3][1]_best_match_text'].value, 0],
                                                                                                self.xanes3D_manual_xshift+shift[self.hs['L[0][2][2][1][3][1]_best_match_text'].value, 1]])
    ...
    def L0_2_3_1_4_1_analysis_run_button(self, a):
        ...
        code['052'] = "    g128 = g12.create_dataset('specturm_edge_jump_filter', shape=(xanes3D_analysis_data_shape[1:]), dtype=np.float32)"
        code['053'] = "    g129 = g12.create_dataset('specturm_edge_offset_filter', shape=(xanes3D_analysis_data_shape[1:]), dtype=np.float32)"
        code['054'] = f"    g12.create_dataset('img_mask', data={np.int8(self.xanes3D_analysis_mask)}, dtype=np.int8)"
        code['055'] = "    for ii in range(xanes3D_analysis_data_shape[1]):"
        code['056'] = "        imgs[:] = f['/registration_results/reg_results/registered_xanes3D'][:, ii, :, :]"
        code['057'] = "        xana.spectrum[:] = imgs[:]"
        code['058'] = "        xana.edge_jump()"
        code['059'] = "        xana.cal_pre_edge_sd()"
        code['060'] = "        xana.cal_post_edge_sd()"
        code['061'] = "        xana.fit_pre_edge()"
        code['062'] = "        xana.fit_post_edge()"
        code['063'] = "        xana.create_edge_jump_filter(xanes3D_analysis_edge_jump_thres)"
        code['064'] = "        xana.create_fitted_edge_filter(xanes3D_analysis_edge_offset_thres)"
        code['065'] = "        xana.normalize_xanes(xanes3D_analysis_edge_eng, order=xanes3D_analysis_norm_fitting_order)"
        code['066'] = "        xana.fit_edge_pos(xanes3D_analysis_edge_0p5_fit_s, xanes3D_analysis_edge_0p5_fit_e, order=3, flt_spec=xanes3D_analysis_use_flt_spec)"
        code['067'] = "        xana.fit_whiteline(xanes3D_analysis_wl_fit_eng_s, xanes3D_analysis_wl_fit_eng_e, order=3, flt_spec=xanes3D_analysis_use_flt_spec)"
        code['068'] = "        xana.calc_direct_whiteline(xanes3D_analysis_wl_fit_eng_s, xanes3D_analysis_wl_fit_eng_e)"
        code['069'] = "        xana.calc_weighted_eng(xanes3D_analysis_pre_edge_e)"
        code['070'] = "        g120[ii] = np.float32(xana.wl_pos)[:]"
        code['071'] = "        g121[ii] = np.float32(xana.wl_pos_direct)[:]"
        code['072'] = "        g122[ii] = np.float32(xana.direct_wl_ph)[:]"
        code['073'] = "        g123[ii] = np.float32(xana.centroid_of_eng)[:]"
        code['074'] = "        g124[ii] = np.float32(xana.centroid_of_eng_rel_wl)[:]"
        code['075'] = "        g125[ii] = np.float32(xana.weighted_atten)[:]"
        code['076'] = "        g126[ii] = np.float32(xana.weighted_eng)[:]"
        code['077'] = "        g127[ii] = np.float32(xana.edge_pos)[:]"
        code['078'] = "        g128[ii] = np.float32(xana.edge_jump_mask)[:]"
        code['079'] = "        g128[ii] = np.float32(xana.fitted_edge_mask)[:]"
        code['080'] = "f.close()"
    ...
    def L0_1_3_1_4_1_analysis_run_button_click(self, a):
        ...
        code['050'] = "    g128 = g12.create_dataset('specturm_edge_jump_filter', shape=(xanes2D_analysis_data_shape[1:]), dtype=np.float32)"
        code['051'] = "    g129 = g12.create_dataset('specturm_edge_offset_filter', shape=(xanes2D_analysis_data_shape[1:]), dtype=np.float32)"
        code['052'] = f"    g12.create_dataset('img_mask', data={np.int8(self.xanes2D_analysis_mask)}, dtype=np.int8)"
        code['053'] = "    xana.edge_jump()"
        code['054'] = "    xana.cal_pre_edge_sd()"
        code['055'] = "    xana.cal_post_edge_sd()"
        code['056'] = "    xana.fit_pre_edge()"
        code['057'] = "    xana.fit_post_edge()"
        code['058'] = "    xana.create_edge_jump_filter(xanes2D_analysis_edge_jump_thres)"
        code['059'] = "    xana.create_fitted_edge_filter(xanes2D_analysis_edge_offset_thres)"
        code['060'] = "    xana.normalize_xanes(xanes2D_analysis_edge_eng, order=xanes2D_analysis_norm_fitting_order)"
        code['061'] = "    xana.fit_edge_pos(xanes2D_analysis_edge_0p5_fit_s, xanes2D_analysis_edge_0p5_fit_e, order=3, flt_spec=xanes2D_analysis_use_flt_spec)"
        code['062'] = "    xana.fit_whiteline(xanes2D_analysis_wl_fit_eng_s, xanes2D_analysis_wl_fit_eng_e, order=3, flt_spec=xanes2D_analysis_use_flt_spec)"
        code['063'] = "    xana.calc_direct_whiteline(xanes2D_analysis_wl_fit_eng_s, xanes2D_analysis_wl_fit_eng_e)"
        code['064'] = "    xana.calc_direct_whiteline_peak_height(xanes2D_analysis_wl_fit_eng_s, xanes2D_analysis_wl_fit_eng_e)"
        code['065'] = "    xana.calc_weighted_eng(xanes2D_analysis_pre_edge_e)"
        code['066'] = "    g120[:] = np.float32(xana.wl_pos)[:]"
        code['067'] = "    g121[:] = np.float32(xana.wl_pos_direct)[:]"
        code['068'] = "    g122[:] = np.float32(xana.direct_wl_ph)[:]"
        code['069'] = "    g123[:] = np.float32(xana.centroid_of_eng)[:]"
        code['070'] = "    g124[:] = np.float32(xana.centroid_of_eng_rel_wl)[:]"
        code['071'] = "    g125[:] = np.float32(xana.weighted_atten)[:]"
        code['072'] = "    g126[:] = np.float32(xana.weighted_eng)[:]"
        code['073'] = "    g127[:] = np.float32(xana.edge_pos)[:]"
        code['074'] = "    g128[:] = np.float32(xana.edge_jump_mask)[:]"
        code['075'] = "    g128[:] = np.float32(xana.fitted_edge_mask)[:]"
        code['076'] = ""
        code['077'] = "f.close()"
