...
class xanes_analysis():
    ...
    def fit_pre_edge(self):
        ...
        """
        try:
            print(self.pre_es_idx, self.pre_ee_idx)
    ...
    def fit_post_edge(self):
        ...
        """
        try:
            # print(self.post_es_idx, self.post_ee_idx)
    ...
    def cal_pre_edge_fit(self, eng):
        ...
        if len(self.pre_edge_fit_coef.shape) == 3:
            pre_edge_fit = (self.pre_edge_fit_coef * \
    ...
    def cal_post_edge_fit(self, eng):
        ...
        if len(self.post_edge_fit_coef.shape) == 3:
            post_edge_fit = (self.post_edge_fit_coef * \
                             (np.vstack((eng, np.ones(eng.shape[0]))).T)\
                                 [:, :, np.newaxis, np.newaxis]).sum(axis=1)
    ...
    def fitted_edges_filter(self, pre_edge_thres):
        """
        inputs:
        ...
        returns:
            mask: array-like; mask in shape of spectrum.shape
        """
        mask = np.any((self.cal_post_edge_fit(self.eng, self.post_edge_fit_coef) -
                       self.cal_pre_edge_fit(self.eng, self.pre_edge_fit_coef)) 
    ...
    def fit_whiteline_poly(self, peak_es, peak_ee, 
        ...
        self.wl_es = self.eng[self.wl_es_idx]
        self.wl_ee = self.eng[self.wl_ee_idx]
        ...
        if flt_spec:
            ...
            flt_kernal[0] = 3
            self.wl_fit_coef = xm.fit_polynd(np.arange(self.wl_es, self.wl_ee),
        ...
        eng = np.linspace(self.wl_es, self.wl_ee,
                          num=(self.wl_ee_idx - self.wl_es_idx + 1)*ufac)
        for ii in range(1, len(self.wl_fit_coef.shape)):
            eng = eng[:, np.newaxis]

        self.wl_pos = xm.find_fit_peak_map_poly(self.wl_fit_coef, eng)
        self.edge_pos = xm.find_deriv_peak_map_poly(self.wl_fit_coef, eng)
        if self.spec_normalized:
            self.edge_pos_0p5 = xm.find_edge_0p5_map_poly(self.wl_fit_coef, eng)
    ...
    def fit_whiteline_scipy(self, peak_es, peak_ee, 
        ...
        self.wl_pos = xm.find_fit_peak_map_scipy(self.wl_model, eng, 
                                                 self.wl_fit_coef).reshape(self.spectrum.shape[1:])
        self.edge_pos = xm.find_deriv_peak_map_scipy(model, eng, self.wl_fit_coef).\
            reshape(self.spectrum.shape[1:])
        if self.spec_normalized:
            self.edge_pos_0p5 = xm.find_fit_edge_0p5_map_scipy(model, eng, self.wl_fit_coef).\
                reshape(self.spectrum.shape[1:])
    ...
    def fit_edge_poly(self, es, ee, 
        ...
        self.edge_pos_0p5_us_idx = xm.index_of(np.polyval(self.edge_fit_coef, us_idx), 0.5)
        self.edge_pos_0p5 = xm.index_lookup(self.edge_pos_0p5_us_idx,
                                        self.eng[idx_s:idx_e],
                                        ufac=ufac)[:]
        self.edge_pos_us_idx = np.argmax(np.gradient(np.polyval(self.edge_fit_coef, us_idx), axis=0)/
    ...
    def fit_edge_scipy(self, es, ee, 
        ...
        us_idx = np.linspace(idx_s, idx_e, num=(idx_e - idx_s + 1)*ufac)
        # for ii in range(1, len(self.edge_fit_coef)):
        #     us_idx = us_idx[:, np.newaxis]

        self.edge_pos_0p5_us_idx = xm.find_edge_0p5_map_scipy(model, us_idx, self.edge_fit_coef)
        self.edge_pos_0p5 = xm.index_lookup(self.edge_pos_0p5_us_idx,
                                            self.eng[idx_s:idx_e],
                                            ufac=ufac).reshape(self.spectrum.shape[1:])[:]
        self.edge_pos_us_idx = xm.find_deriv_peak_map_scipy(model, us_idx, self.edge_fit_coef)
        self.edge_pos = xm.index_lookup(self.edge_pos_us_idx,
                                        self.eng[idx_s:idx_e],
                                        ufac=ufac).reshape(self.spectrum.shape[1:])[:]
        
    def fit_edge(self, es, ee, method, **kwargs):
        if method == 'scipy':
