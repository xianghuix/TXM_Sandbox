...
class regtools():
    ...
    def set_cal_anchor(self, img_id_s, img_id_e, fixed_img_id, dtype='3D_XANES'):
        """
        Parameters
        ----------
        img_id_s : integer
    ...
    def _sort_absolute_shift(self, shift_dict, trialfn, optional_shift_dict=None):
                ...
                if self.mask is not None:
                    for ii in range(len(self.alignment_pair)):
                        self.shift[ii] = sr.register(self.moving[self.alignment_pair[ii][0]]*self.mask[0],
                                                     self.moving[self.alignment_pair[ii][1]]*self.mask[0])
                        shifted_image[ii] = sr.transform(self.moving[self.alignment_pair[ii][1]],
                                                                               tmat=self.shift[ii])[:]
                        print(ii, self.shift[ii])
                else:
                    for ii in range(len(self.alignment_pair)):
    ...
    def reg_xanes2D_chunk(self, overlap_ratio=0.3, method=None,
                          ref_mode=None, is_raw=False, save=True):
        """
    ...
    def apply_xanes2D_chunk_shift(self, trialfn=None, savefn=None, optional_shift_dict=None):
        ...
        if savefn == trialfn:
            ...
            self._sort_absolute_shift(shift_dict, trialfn)
    ...
    def apply_xanes3D_chunk_shift(self, shift_dict, sli_s, sli_e,
        ...
        if savefn == trialfn:
            ...
            dicttoh5(shift_dict, savefn, mode='a',
                     overwrite_data=True,
