...
class regtools():
    ...
    # def set_cal_anchor(self, dtype='3D_XANES'):
    #     """
    #     Parameters
    #     ----------
    #     img_id_s : integer
    ...
    #             self.img_id = np.arange(self.img_id_s, self.img_id_e+1)
    ...
    #         #     f.close()
    #         # self.img = np.ndarray([self.img_id_e-self.img_id_s,
    #         #                           self.roi[1]-self.roi[0],
    #         #                           self.roi[3]-self.roi[2]])
    #         if self.use_mask:
    #             if self.use_smooth_img:
    #                 f = h5py.File(self.xanes2D_raw_filename, 'r')
    #                 self.xanes2D_img = f[]
    ...
    #         self.img = np.ndarray([2*self.xanes3D_sli_search_half_range,
    #                                   self.roi[1]-self.roi[0],
    #                                   self.roi[3]-self.roi[2]])
    ...
    def compose_dicts(self):
        if self.data_type == '3D_XANES':
            ...
            if self.fixed_img_id in range(self.img_id_s, self.img_id_e+1):
                ...
                cnt = 0
                for ii in range(self.img_id_s, self.img_id_e+1):
                    self.eng_dict[str(cnt).zfill(3)] = self.eng_list[ii-self.img_id_s]
            ...
            if method.upper() == "SR":
                for key, item in self.shift_chain_dict.items():
                    ...
                    for ii in range(len(item)-1):
                        idx = self.alignment_pair_list.index([item[ii+1], item[ii]])
                        if optional_shift_dict is None:
                            h5path = '/trial_registration/trial_reg_results/{0}/shift{0}'.format(str(idx).zfill(3))
                            shift *= f[h5path][:]
                        else:
                            if str(idx) in optional_shift_dict.keys():
                ...
                for key, item in self.shift_chain_dict.items():
                    ...
                    for ii in range(len(item)-1):
                        ...
                        if optional_shift_dict is None:
                            ...
                            if str(idx) in optional_shift_dict.keys():
                                print('optional_shift_dict')
                                print(shift)
                                print(optional_shift_dict[str(idx)])
                                shift += optional_shift_dict[str(idx)]
                            else:
                                print('noop idx:', idx)
                                h5path = '/trial_registration/trial_reg_results/{0}/shift{0}'.format(str(idx).zfill(3))
    ...
    def _reg_xanes2D_chunk(self):
        ...
        if self.img.ndim != 3:
            ...
            if self.method.upper() == 'PC':
                ...
                if self.mask is not None:
                    for ii in range(len(self.alignment_pair_list)):
                        self.shift[ii], self.error[ii], _ = register_translation(
                                self.img[self.alignment_pair_list[ii][0]]*self.mask,
                                self.img[self.alignment_pair_list[ii][1]]*self.mask, 100)
                    ...
                    for ii in range(len(self.alignment_pair_list)):
                        self.shift[ii], self.error[ii], _ = register_translation(
                                ...
                                self.img[self.alignment_pair_list[ii][1]], 100)
                        shifted_image[ii] = np.real(np.fft.ifftn(fourier_shift(
                                np.fft.fftn(self.img[self.alignment_pair_list[ii][1]]), self.shift[ii])))[:]
                        g11 = g1.create_group(str(ii).zfill(3))
                        g11.create_dataset('shift'+str(ii).zfill(3),
                ...
                for ii in range(len(self.alignment_pair_list)):
                    self.shift[ii] = masked_register_translation(self.img[self.alignment_pair_list[ii][0]],
    ...
    def apply_xanes2D_chunk_shift(self, trialfn=None, savefn=None, optional_shift_dict=None):
        ...
        if savefn == trialfn:
            ...
            for key in sorted(self.abs_shift_dict.keys()):
                ...
                self._translate_single_img(self.img[int(key)][self.roi[0]-yshift_int:self.roi[1]-yshift_int,
                                                                 self.roi[2]-xshift_int:self.roi[3]-xshift_int],
                                           shift, self.method)
                g21[cnt1] = self.img[int(key)][self.roi[0]-yshift_int:self.roi[1]-yshift_int,
                                               self.roi[2]-xshift_int:self.roi[3]-xshift_int]
            ...
            if self.mask is not None:
                for ii in range(len(self.alignment_pair_list)):
                    ...
                    for jj in range(self.xanes3D_recon_fixed_sli-self.xanes3D_sli_search_half_range,
                        ...
                        self.img[jj_id] = tifffile.imread(fn)[self.roi[0]:self.roi[1], self.roi[2]:self.roi[3]]

                        self.shift[ii, jj_id], self.error[ii, jj_id], _ = register_translation(self.fixed*self.mask,
                                                                                 self.img[jj_id]*self.mask, 100)
                        self.img[jj_id] = np.real(np.fft.ifftn(fourier_shift(np.fft.fftn(self.img[jj_id]),
                                                                                         self.shift[ii, jj_id])))[:]
            ...
            if self.mask is not None:
                for ii in range(len(self.alignment_pair_list)):
                    ...
                    for jj in range(self.xanes3D_recon_fixed_sli-self.xanes3D_sli_search_half_range,
                        ...
                        self.shift[ii, jj_id] = masked_register_translation(self.fixed, self.img[jj_id],
            ...
            if self.mask is not None:
                for ii in range(len(self.alignment_pair_list)):
                    ...
                    for jj in range(self.xanes3D_recon_fixed_sli-self.xanes3D_sli_search_half_range,
                        ...
                        self.shift[ii, jj_id] = sr.register(self.fixed*self.mask, self.img[jj_id]*self.mask)
                        self.img[jj_id] = sr.transform(self.img[jj_id], self.shift[ii, jj_id])[:]
                ...
                for ii in range(len(self.alignment_pair_list)):
                    ...
                    for jj in range(self.xanes3D_recon_fixed_sli-self.xanes3D_sli_search_half_range,
                        ...
                        self.shift[ii, jj_id] = sr.register(self.fixed, self.img[jj_id])
                        self.img[jj_id] = sr.transform(self.img[jj_id], self.shift[ii, jj_id])[:]
    ...
    #     if self.method.upper() in {'PC', 'MPC'}:
    #         for ii in range(len(self.alignment_pair_list)):
    #             self.img[self.alignment_pair_list[ii][1]] = np.real(np.fft.ifftn(fourier_shift(
    #                     np.fft.fftn(self.img[self.alignment_pair_list[ii][1]]), self.shift[ii])))[:]
    ...
    #             sr = StackReg(StackReg.BILINEAR)
    #         for ii in range(len(self.alignment_pair_list)):
    #             self.img[self.alignment_pair_list[ii][1]] = sr.transform(self.img[self.alignment_pair_list[ii][1]],
    #                                                                    tmat=self.shift[ii])[:]
    #             print(ii, self.shift[ii])
